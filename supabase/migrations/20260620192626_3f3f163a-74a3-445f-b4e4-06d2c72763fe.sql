-- 1) Geração inicial dos cards: pacotes ficam como "Cliente • Vídeo 01"
...
BEGIN
  ...
  FOR i IN 1..GREATEST(NEW.service_quantity, 1) LOOP
    IF is_pkg THEN
      card_title := COALESCE(customer_name, 'Cliente') || ' • Vídeo ' || LPAD(i::text, 2, '0');
    ELSE
      card_title := COALESCE(customer_name, 'Cliente') || ' • ' || COALESCE(st_name, 'Serviço') || ' #' || i;
    END IF;
...
-- 2) Sincronização de edição da venda: preserva numeração por card
...
BEGIN
  ...
  SET
    title = CASE
      WHEN v_is_pkg THEN
        COALESCE(v_customer_name, 'Cliente') || ' • Vídeo ' || LPAD(COALESCE(so.service_index, 1)::text, 2, '0')
      ELSE
        COALESCE(v_customer_name, 'Cliente') || ' • ' || COALESCE(v_service_name, 'Serviço') || ' #' || COALESCE(so.service_index, 1)
    END,
