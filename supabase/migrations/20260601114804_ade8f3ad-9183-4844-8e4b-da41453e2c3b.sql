...
-- Security-definer function that resets all operational data, callable by admins only.
...
BEGIN
  IF uid IS NULL OR NOT public.has_role(uid, 'admin'::app_role) THEN
    RAISE EXCEPTION 'Apenas administradores podem resetar a plataforma' USING ERRCODE = '42501';
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_reset_platform() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_reset_platform() TO authenticated;
