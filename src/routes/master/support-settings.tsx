...
function Page() {
  ...
  useEffect(() => {
    supabase.from("internal_config").select("value").eq("key", "support_contacts").maybeSingle()
      .then(({ data }) => {
        if (data?.value) { try { setContacts(JSON.parse(data.value) as SupportContacts); } catch { /* ignore */ } }
        setLoading(false);
      });
  }, []);
  ...
  const save = async () => {
    ...
    setSaving(false);
    if (error) toast.error(error.message); else toast.success("Contatos salvos com sucesso");
  };
          ...
          <CardContent className="p-6 space-y-5">
            ...
            <div className="flex justify-end">
              <Button onClick={save} disabled={saving} size="lg" className="gap-2">
                {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Salvar contatos
              </Button>
            </div>
          </CardContent>
        </Card>
