...
} from "@/lib/email-config.functions";
  ...
  async function onTest(kind: "signup" | "reset") {
    ...
    try {
      ...
      setLastTest({
        ...
        message:
          "A Brevo aceitou o envio. Se não aparecer na caixa de entrada, verifique Spam/Promoções ou o status do remetente na Brevo.",
    ...
    <div className="mx-auto max-w-4xl space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Envio via <strong>Brevo</strong> — cadastro e recuperação de senha
          </p>
        </div>
      </div>
          ...
          {hasKey ? (
            <>
              ...
              <div className="text-sm">
                Chave <code className="rounded bg-muted px-1.5 py-0.5">BREVO_API_KEY</code>{" "}
                configurada.
              </div>
          ...
          <CardContent className="flex items-start gap-3 py-4 text-sm">
            ...
            <div>
              <div className="font-medium">
                Status Brevo: {brevoStatus.ok ? "Pronto" : "Atenção"}
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            O e-mail precisa estar verificado na Brevo (Senders & IPs).
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Assunto</Label>
            <Input
              value={form.signup_subject}
              onChange={(e) => update("signup_subject", e.target.value)}
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              placeholder="https://.../banner-cadastro.png"
              value={form.signup_banner_url ?? ""}
              onChange={(e) => update("signup_banner_url", e.target.value)}
            />
            {form.signup_banner_url && (
              <img
                src={form.signup_banner_url}
                alt="preview"
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Switch
              checked={form.reset_enabled}
              onCheckedChange={(v) => update("reset_enabled", v)}
            />
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Assunto</Label>
            <Input
              value={form.reset_subject}
              onChange={(e) => update("reset_subject", e.target.value)}
            />
          </div>
          <div className="space-y-2">
            <Label className="flex items-center gap-1.5">
            ...
            <Input
              placeholder="https://.../banner-reset.png"
              value={form.reset_banner_url ?? ""}
              onChange={(e) => update("reset_banner_url", e.target.value)}
            />
            {form.reset_banner_url && (
              <img
                src={form.reset_banner_url}
                alt="preview"
                className="mt-2 max-h-40 rounded border"
              />
            )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Envie um e-mail de teste para conferir remetente e banner.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() => onTest("signup")}
              disabled={testing !== null || !hasKey}
            >
              <Send className="h-4 w-4 mr-2" />{" "}
              {testing === "signup" ? "Enviando…" : "Testar Cadastro"}
            </Button>
            <Button
              variant="outline"
          ...
          {lastTest && (
            <div
              className={
                lastTest.ok
                  ? "rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3 text-sm"
                  : "rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm"
              }
            >
              <div className="font-medium">
                {lastTest.ok ? "Envio aceito pela Brevo" : "Falha no envio"} —{" "}
                {lastTest.kind === "signup" ? "Cadastro" : "Reset"}
              </div>
              <div className="mt-1 text-muted-foreground">
                Destino: {lastTest.to} • {lastTest.at}
              </div>
              {lastTest.messageId && (
                <div className="mt-1 break-all text-muted-foreground">
                  ID Brevo: {lastTest.messageId}
                </div>
              )}
