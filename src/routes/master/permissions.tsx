...
import {
  masterDeleteUser,
  masterGenerateAccessLink,
  masterSetBlocked,
} from "@/lib/master-users.functions";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function Page() {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await supabase.rpc("master_list_users_with_roles", {
  ...
  async function removeUser(u: Row) {
    if (!confirm(`Excluir DEFINITIVAMENTE a conta ${u.email}? Esta ação não pode ser desfeita.`))
      return;
      ...
      <Card className="border-primary/20">
        <CardContent className="pt-6 space-y-4">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {rows.map((u) => (
                <div
                  key={u.user_id}
                  className="rounded-lg border border-border/60 p-3 flex flex-col sm:flex-row sm:items-center gap-3"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Select
                      value={newRole[u.user_id] ?? ""}
                      onValueChange={(v) => setNewRole((s) => ({ ...s, [u.user_id]: v as Role }))}
                    >
                    ...
                    >
                      {busy === u.user_id + newRole[u.user_id] ? (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      title="Acessar conta (magic link)"
                      onClick={() => impersonate(u)}
                      disabled={busy === u.user_id + "imp"}
                    >
                      {busy === u.user_id + "imp" ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      title={u.status === "suspended" ? "Reativar login" : "Bloquear login"}
                      onClick={() => toggleBlock(u)}
                      disabled={busy === u.user_id + "block"}
                    >
                      {busy === u.user_id + "block" ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : u.status === "suspended" ? (
          ...
          <p className="text-xs text-muted-foreground pt-2">
            <strong>Master:</strong> acesso total. <strong>Admin:</strong> gerencia clientes, planos
            e configurações.
            <strong> Supervisor/Atendente/Viewer:</strong> reservados para permissões futuras por
            módulo.
          </p>
        </CardContent>
      </Card>
      ...
      <Dialog open={!!accessLink} onOpenChange={(v) => !v && setAccessLink(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Abra o link em uma aba anônima para entrar na conta de{" "}
              <strong>{accessLink?.email}</strong>. O link é de uso único e expira em minutos.
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAccessLink(null)}>
              Fechar
            </Button>
            {accessLink?.link && (
              <Button asChild>
                <a href={accessLink.link} target="_blank" rel="noreferrer">
                  Abrir agora
                </a>
              </Button>
            )}
