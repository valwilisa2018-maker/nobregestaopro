...
function Page() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (v?.value) {
        try {
          setVideos(JSON.parse(v.value) as Tutorials);
        } catch {
          /* ignore */
        }
      }
      if (c?.value) {
        try {
          setCovers(JSON.parse(c.value) as Tutorials);
        } catch {
          /* ignore */
        }
      }
  ...
  const save = async () => {
    ...
    else toast.success("Tutoriais salvos");
  };
  ...
  const addModule = () => {
    ...
    setModules((prev) => [
      ...prev,
      {
        key,
        label: "Novo módulo",
        subtitle: `Módulo ${String(n).padStart(2, "0")}`,
  ...
  const onFile = (key: string, f: File | null) => {
    ...
    if (f.size > MAX_BYTES) {
      ...
      toast.error(`Arquivo muito grande (${mb}MB)`, {
        description: "Tamanho máximo permitido: 2MB. Reduza a imagem ou informe uma URL.",
      });
      return;
    }
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        if (ratio > 0.85) {
          toast.warning("Proporção fora do padrão 9:16", {
            description: `A capa tem ${img.width}×${img.height}px. O ideal é vertical (ex.: 720×1280).`,
          });
        }
        setCovers((c) => ({ ...c, [key]: String(reader.result) }));
        toast.success("Capa carregada", {
          description: `${f.name} · ${(f.size / 1024).toFixed(0)} KB`,
        });
      };
      img.onerror = () =>
        toast.error("Não foi possível ler a imagem", {
          description: "Arquivo corrompido ou formato não suportado.",
        });
      img.src = String(reader.result);
    };
    reader.onerror = () =>
      toast.error("Falha ao ler o arquivo", {
        description: "Tente novamente ou use outro arquivo.",
      });
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center justify-center gap-1 rounded-md border border-input bg-background px-2 py-1.5 text-xs cursor-pointer hover:bg-accent">
            ...
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={(e) => onFile(m.key, e.target.files?.[0] ?? null)}
            />
          </label>
        </div>
        <div className="space-y-2">
          <div className="grid grid-cols-[110px_1fr] gap-2">
            <div className="space-y-1">
              ...
              <Input
                value={m.subtitle}
                onChange={(e) => onPatch(m.key, { subtitle: e.target.value })}
            ...
            <div className="space-y-1">
              ...
              <Input
                value={m.label}
                onChange={(e) => onPatch(m.key, { label: e.target.value })}
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              value={videoUrl}
              onChange={(e) => onVideo(m.key, e.target.value)}
            />
          </div>
          <div className="space-y-1">
            ...
            <Input
              ...
              value={coverUrl}
              onChange={(e) => onCoverUrl(m.key, e.target.value)}
            />
          </div>
        </div>
      </div>
