...
function Kpi({
  ...
  return (
    <Card className="relative overflow-hidden">
      <div
        className={`absolute inset-0 bg-gradient-to-br ${tones[tone]} opacity-40 pointer-events-none`}
      />
      <CardContent className="relative p-4 flex items-start gap-3">
        <div
          className={`grid h-10 w-10 place-items-center rounded-xl bg-background/60 ring-1 ${tones[tone].split(" ").slice(-1)}`}
        >
          {icon}
        </div>
...
function statusBadge(s: string | null) {
  ...
  if (s === "suspended")
    return <Badge className="bg-rose-500/15 text-rose-500 border-rose-500/30">Suspenso</Badge>;
...
function Page() {
  ...
  const load = useCallback(async () => {
    ...
    const [o, p, pl, w, r, pk] = await Promise.all([
      ...
      supabase
        .from("profiles")
        .select(
          "id,full_name,phone,status,plan_id,plan_expires_at,plan_activated_at,suspended_reason",
        )
        .order("created_at", { ascending: false })
        .limit(1000),
      supabase
        .from("plans")
        .select("id,name,tokens_included,price_cents")
        .eq("is_active", true)
        .order("sort_order"),
      supabase
        .from("credit_wallets")
        .select("user_id,plan_tokens_remaining,extra_tokens_remaining"),
      supabase
        .from("plan_activation_requests")
        .select("id,user_id,plan_id,status,created_at,note")
        .order("created_at", { ascending: false })
        .limit(500),
      supabase
        .from("credit_packages")
        .select("id,name,tokens,price_cents")
        .eq("is_active", true)
        .order("price_cents"),
    ]);
  ...
  const soon = profiles.filter(
    (p) =>
      p.plan_expires_at &&
      new Date(p.plan_expires_at).getTime() - Date.now() < 3 * 86400000 &&
      new Date(p.plan_expires_at).getTime() > Date.now(),
  ).length;
  const expired = profiles.filter(
    (p) => p.plan_expires_at && new Date(p.plan_expires_at).getTime() < Date.now(),
  ).length;
  ...
  const approveRequest = async (r: PlanRequest, days = 30) => {
    setBusy(r.id);
    const { error } = await sbRpc("master_approve_plan_request", {
      _request_id: r.id,
      _days: days,
    });
    setBusy(null);
  ...
  const cancelPlan = async (p: Profile) => {
    ...
    setBusy(p.id);
    const { error } = await sbRpc("master_cancel_plan", {
      _user_id: p.id,
      _reason: "cancelado pelo admin",
    });
    setBusy(null);
    if (error) return toast.error(error.message);
  ...
  const doGrant = async () => {
    ...
    const { error } = await sbRpc("master_grant_credits", {
      _user_id: grantFor.id,
      _tokens: grantTokens,
      _reason: grantReason || "concessão manual",
    });
    setBusy(null);
    if (error) return toast.error(error.message);
  ...
  const doSuspend = async () => {
    ...
    const { error } = await sbRpc("master_suspend_account", {
      _user_id: suspendFor.id,
      _reason: suspendReason || "sem motivo",
    });
    setBusy(null);
    if (error) return toast.error(error.message);
  ...
  const doActivateByEmail = async () => {
    ...
    const pkgTokens =
      emailPkg !== "__none" ? (packages.find((p) => p.id === emailPkg)?.tokens ?? 0) : 0;
    ...
    setBusy("email");
    try {
      const res = (await activateByEmailFn({
        data: { email, planId, days: emailDays, tokens },
      })) as { fullName: string | null };
      toast.success(`Ativação concluída para ${res.fullName ?? email}`);
      setEmailInput("");
      setEmailPlan("__none");
      setEmailPkg("__none");
      setEmailCustomTokens(0);
      load();
    } catch (e) {
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <Kpi
          ...
          value={nf(pending.length)}
          sub={brl(pendingValue)}
          tone="amber"
        />
        <Kpi
        ...
        <TabsList>
          ...
          <TabsTrigger value="requests">
            ...
            {pendingRequests.length > 0 && (
              <Badge className="ml-2 bg-sky-500/20 text-sky-500 border-sky-500/30">
                  ...
                  {pending.map((o) => {
                    ...
                    return (
                      <tr key={o.id} className="border-t">
                        ...
                        <td className="p-3">
                          <Coins className="inline h-3.5 w-3.5 mr-1 text-amber-500" />
                          {nf(o.tokens)}
                        </td>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
                  <tr>
                    <th className="text-left p-3">Data</th>
                    <th className="text-left p-3">Cliente</th>
                    <th className="text-left p-3">Plano solicitado</th>
                    <th className="text-left p-3">Valor</th>
                    <th className="text-right p-3">Ação</th>
                  </tr>
                </thead>
                      ...
                      <tr key={r.id} className="border-t">
                        ...
                        <td className="p-3">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {u?.phone ?? r.user_id.slice(0, 8)}
                          </div>
                        </td>
                        <td className="p-3">
                          <div className="font-medium">{pl?.name ?? "—"}</div>
                          <div className="text-xs text-muted-foreground">
                            {nf(pl?.tokens_included ?? 0)} tokens
                          </div>
                        </td>
            ...
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Cole o e-mail do cliente, escolha o plano e/ou créditos e ative na hora.
                  </p>
                </div>
              </div>
              <div className="grid gap-3 md:grid-cols-[2fr_2fr_1fr_2fr_auto]">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      setEmailPkg(v);
                      if (v !== "__none") setEmailCustomTokens(0);
                    }}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      {packages.map((p) => (
                        <SelectItem key={p.id} value={p.id}>
                          {p.name} — {nf(p.tokens)} tokens · {brl(p.price_cents)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              {emailPkg === "__none" && (
                <div className="grid gap-2 md:grid-cols-[1fr_auto]">
                  <div>
                    <label className="text-[10px] uppercase text-muted-foreground">
                      Ou quantidade personalizada de tokens
                    </label>
                    <Input
                      type="number"
                      min={0}
                      value={emailCustomTokens}
                      onChange={(e) => setEmailCustomTokens(Number(e.target.value))}
                  ...
                  <div className="flex items-end gap-2 flex-wrap">
                    {[10000, 100000, 1000000, 20000000].map((v) => (
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={() => setEmailCustomTokens(v)}
                      >
                        {nf(v)}
                      </Button>
                    ))}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredProfiles.slice(0, 200).map((p) => {
                    ...
                    const total =
                      (w?.plan_tokens_remaining ?? 0) + (w?.extra_tokens_remaining ?? 0);
                    return (
                      <tr key={p.id} className="border-t">
                        ...
                        <td className="p-3 text-xs">
                          {p.plan_expires_at
                            ? new Date(p.plan_expires_at).toLocaleDateString("pt-BR")
                            : "—"}
                        </td>
                        ...
                        <td className="p-3 text-right space-x-1">
                          <Button
                            ...
                            onClick={() => {
                              setActivateFor(p);
                              setActPlan(p.plan_id ?? plans[0]?.id ?? "");
                              setActDays(30);
                            }}
                          >
      ...
      <Dialog open={!!activateFor} onOpenChange={(v) => !v && setActivateFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Ativar conta — {activateFor?.full_name ?? ""}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Select value={actPlan} onValueChange={setActPlan}>
                ...
                <SelectContent>
                  {plans.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      {p.name} — {brl(p.price_cents)} · {nf(p.tokens_included ?? 0)} tokens
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                Vence em {new Date(Date.now() + actDays * 86400000).toLocaleDateString("pt-BR")}
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setActivateFor(null)}>
              Cancelar
            </Button>
            <Button onClick={doActivate} disabled={busy === "activate" || !actPlan}>
              {busy === "activate" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Ativar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!grantFor} onOpenChange={(v) => !v && setGrantFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Conceder tokens — {grantFor?.full_name ?? ""}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {[10000, 50000, 100000, 500000, 1000000].map((v) => (
                  <Button key={v} size="sm" variant="outline" onClick={() => setGrantTokens(v)}>
                    {nf(v)}
                  </Button>
                ))}
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={doGrant} disabled={busy === "grant" || !grantTokens}>
              {busy === "grant" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Conceder"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!suspendFor} onOpenChange={(v) => !v && setSuspendFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Suspender conta — {suspendFor?.full_name ?? ""}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setSuspendFor(null)}>
              <X className="h-4 w-4 mr-1" />
              Cancelar
            </Button>
            <Button variant="destructive" onClick={doSuspend} disabled={busy === "suspend"}>
              {busy === "suspend" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Suspender"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
