...
const STATUS_META: Record<string, { label: string; color: string; cls: string }> = {
  ...
  pending: {
    ...
    color: "#06b6d4",
    cls: "bg-cyan-500/15 text-cyan-500 border-cyan-500/40",
  },
  waiting_dev: {
  ...
  cancelled: {
    label: "Cancelado",
    color: "#ef4444",
    cls: "bg-red-500/15 text-red-500 border-red-500/40",
  },
};
const PRIORITIES = [
...
function Page() {
  ...
  const stats = useMemo(() => {
    ...
    const avgResp = responded.length
      ? responded.reduce(
          (s, t) =>
            s + (new Date(t.first_response_at!).getTime() - new Date(t.created_at).getTime()),
          0,
        ) / responded.length
      : 0;
    ...
    const avgRes = resolved.length
      ? resolved.reduce(
          (s, t) => s + (new Date(t.resolved_at!).getTime() - new Date(t.created_at).getTime()),
          0,
        ) / resolved.length
      : 0;
    ...
    return {
      ...
      urgent: tickets.filter(
        (t) => t.priority === "urgent" && !["resolved", "closed", "cancelled"].includes(t.status),
      ).length,
  ...
  const priorityBar = useMemo(
    () =>
      PRIORITIES.map((p) => ({
        name: p.label,
        total: tickets.filter((t) => t.priority === p.value).length,
        color: p.color,
      })),
    [tickets],
  );
  ...
  return (
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <Button asChild variant="outline" size="sm" className="gap-2">
            <Link to="/master/support-settings">
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <BigKpi
          label={`Novos (últimos ${range}d)`}
          value={stats.newInRange}
          tone="blue"
          icon={<TrendingUp className="h-5 w-5" />}
          sub={`${stats.total} chamados totais`}
        />
        <BigKpi
          label="Abertos + em andamento"
          value={stats.open + stats.inProgress}
          tone="amber"
          icon={<Activity className="h-5 w-5" />}
          sub={`${stats.pending} aguardando cliente`}
        />
        <BigKpi
        ...
        <BigKpi
          ...
          sub={`${stats.uniqueClients} clientes atendidos`}
        />
      </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold flex items-center gap-2">
                <TrendingUp className="h-4 w-4 text-primary" /> Tendência de chamados
              </h3>
        ...
        <Card className="border-border/60 bg-gradient-to-br from-card to-card/50">
          <CardContent className="p-5">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" /> Distribuição por status
            </h3>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-border/60 bg-gradient-to-br from-card to-card/50">
          <CardContent className="p-5">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-orange-500" /> Chamados por prioridade
            </h3>
        ...
        <Card className="border-border/60 bg-gradient-to-br from-card to-card/50">
          <CardContent className="p-5">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Users className="h-4 w-4 text-primary" /> Top categorias
            </h3>
      ...
      <Card>
        <CardContent className="p-4 flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <Input
              placeholder="Buscar por título, #número ou ID do cliente"
              className="pl-9"
              value={search}
              ...
              {!selected ? (
                ...
                <>
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-3">
                    <div className="min-w-0">
                      <div className="text-[10px] font-mono text-muted-foreground">
                        #{String(selected.ticket_number ?? "—").padStart(6, "0")}
                      </div>
                      <h3 className="font-semibold truncate">{selected.subject}</h3>
                      <p className="text-xs text-muted-foreground">
                        Cliente {selected.user_id.slice(0, 8)} · Aberto em{" "}
                  ...
                  <div className="flex-1 overflow-y-auto py-3 space-y-2">
                    {messages.map((m) => (
                      ...
                      >
                        <div
                          className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-sm shadow-sm ${m.sender_role === "master" ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-muted rounded-bl-sm"}`}
                        >
                    ...
                    {messages.length === 0 && (
                      <div className="grid place-items-center text-xs text-muted-foreground py-8">
