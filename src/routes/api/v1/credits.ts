...
export const Route = createFileRoute("/api/v1/credits")({
  server: { handlers: { GET: async ({ request }) => {
    ...
    const { data: before } = await ctx.supabase.from("credit_wallets").select("plan_tokens_reset_at").eq("user_id", ctx.userId).maybeSingle();
    ...
    if (before?.plan_tokens_reset_at && data?.plan_tokens_reset_at && before.plan_tokens_reset_at !== data.plan_tokens_reset_at) {
