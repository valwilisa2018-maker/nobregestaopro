                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">
                        {r.last_used_at ? new Date(r.last_used_at).toLocaleString("pt-BR") : "—"}
                      </td>
                      <td className="px-4 py-3">
                        {r.revoked_at ? (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Nome descritivo para identificar onde a chave será usada.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={create} disabled={saving}>
              {saving && <Loader2 className="h-4 w-4 animate-spin" />} Gerar Chave
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <KeyRound className="h-5 w-5 text-primary" /> Copie sua chave agora
            </DialogTitle>
            <DialogDescription>
              Esta é a única vez que a chave completa será exibida. Guarde em local seguro.
            </DialogDescription>
          </DialogHeader>
