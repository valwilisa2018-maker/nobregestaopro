...
import {
  ...
  deleteExecution,
  validateFlow,
} from "@/lib/flow-simulator.functions";
...
function Page() {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: `execution_id=eq.${selected.id}`,
        },
        (payload) => {
  ...
  const liveErrors = useMemo(
    () => liveExecs.filter((e) => e.status === "failed" || e.last_error).length,
    [liveExecs],
  );
  ...
  const statusBadge = (s: string) => {
    ...
    return (
      <Badge variant="outline" className={`${it.cls} gap-1`}>
        {it.icon}
        {it.label}
      </Badge>
    );
  ...
  const renderEvent = (ev: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 text-[10px] font-medium uppercase ${it.cls}`}
      >
        {it.icon}
        {it.label}
      </span>
  ...
  const renderList = (rows: ExecRow[], emptyMsg: string) => (
    <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden">
      <div className="border-b border-border/60 px-3 py-2 text-xs text-muted-foreground flex items-center justify-between">
        <span>
          {rows.length} execuç{rows.length === 1 ? "ão" : "ões"}
        </span>
        <button
          onClick={() => void reload()}
      ...
      <div className="max-h-[70vh] overflow-y-auto divide-y divide-border/50">
        ...
        {!loading && !rows.length && (
          <div className="p-6 text-center text-xs text-muted-foreground">{emptyMsg}</div>
        )}
        {rows.map((e) => (
          <button
            key={e.id}
            onClick={() => setSelected(e)}
            className={`w-full text-left p-3 hover:bg-accent/30 transition ${selected?.id === e.id ? "bg-accent/40" : ""}`}
          >
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium truncate flex-1">
                {flowName[e.flow_id] ?? e.flow_id.slice(0, 8)}
              </span>
  ...
  const renderDetail = () => (
    <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden flex flex-col">
      {!selected ? (
        <div className="grid place-items-center h-[70vh] text-sm text-muted-foreground">
        ...
        <>
          <div className="border-b border-border/60 px-4 py-3 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate">
                {flowName[selected.flow_id] ?? selected.flow_id}
              </div>
              <div className="text-[11px] text-muted-foreground flex items-center gap-2">
                {statusBadge(selected.status)}
                {selected.awaiting_variable && (
                  <span>
                    aguardando: <code>{selected.awaiting_variable}</code>
                  </span>
                )}
                {selected.current_block_id && (
                  <span>
                    · bloco: <code>{selected.current_block_id}</code>
                  </span>
                )}
              </div>
            </div>
            <Button variant="ghost" size="icon" onClick={() => onDelete(selected.id)}>
          ...
          {selected.variables && Object.keys(selected.variables).length > 0 && (
            <div className="border-b border-border/60 px-4 py-2 text-xs">
              ...
              {Object.entries(selected.variables).map(([k, v]) => (
                <Badge key={k} variant="outline" className="mr-1 mb-1 font-mono text-[10px]">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1 max-h-[55vh]">
            {logs.map((l) => (
              <div
                key={l.id}
                className={`text-xs rounded px-2 py-1.5 border ${l.level === "error" ? "border-red-500/30 bg-red-500/5" : l.level === "warn" ? "border-yellow-500/30 bg-yellow-500/5" : "border-border/40 bg-muted/20"}`}
              >
                <div className="flex items-center gap-2">
                  ...
                  {renderEvent(l.event)}
                  {l.block_id && (
                    <code className="text-[10px] text-muted-foreground">{l.block_id}</code>
                  )}
                  {l.duration_ms != null && (
                ...
                {l.message && (
                  <div className="mt-0.5 whitespace-pre-wrap break-words">{l.message}</div>
                )}
                {l.data && Object.keys(l.data).length > 0 && (
                  <pre className="mt-1 text-[10px] text-muted-foreground bg-background/5 rounded p-1 overflow-x-auto">
          ...
          {selected.is_simulation && selected.status === "waiting_user_input" && (
            <div className="border-t border-border/60 p-3 flex gap-2">
              <Input
                placeholder={`Digite a resposta para "${selected.awaiting_variable ?? "usuário"}"…`}
                value={input}
  ...
  const activeExecs = execs.filter(
    (e) => e.status === "processing" || e.status === "waiting_user_input",
  ).length;
  ...
  const kpis = [
    {
      label: "Execuções ao vivo",
      value: liveExecs.length,
      icon: Activity,
    ...
    },
    {
      ...
      icon: CheckCircle2,
      grad: "from-violet-500/25 to-fuchsia-500/10",
      ring: "ring-violet-500/30",
      tone: "text-violet-500",
    },
    {
      label: "Erros",
      value: liveErrors + diagErrors,
      icon: AlertTriangle,
  ...
  return (
    <PageShell>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-indigo-600/20 via-violet-600/10 to-fuchsia-600/20 p-6 mb-5 shadow-[0_10px_40px_-15px_rgba(99,102,241,0.35)]">
        <div
          className="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-violet-500/20 blur-3xl"
          aria-hidden
        />
        <div
          className="absolute -bottom-24 -left-16 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl"
          aria-hidden
        />
        <div className="relative flex flex-col md:flex-row md:items-center gap-5">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div
                className="absolute inset-0 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 blur-md opacity-70"
                aria-hidden
              />
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-xl">
                Monitore execuções reais, simule fluxos como se fosse o contato e diagnostique por
                que uma mensagem não disparou um fluxo.
              </p>
            </div>
          </div>
        ...
        <div className="relative mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
          {kpis.map((k) => (
            <div
              key={k.label}
              className={`relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br ${k.grad} p-4 ring-1 ${k.ring}`}
            >
              <div className="flex items-center justify-between">
                ...
                <k.icon
                  className={`h-4 w-4 ${k.tone} ${k.label === "Ativas agora" && activeExecs > 0 ? "animate-spin" : ""}`}
                />
              </div>
      ...
      <Tabs
        value={tab}
        onValueChange={(v) => setTab(v as "live" | "sim" | "diag")}
        className="w-full"
      >
        <TabsList className="mb-6 inline-flex h-auto flex-wrap gap-1.5 rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 via-slate-900/60 to-slate-950/80 p-1.5 shadow-[0_8px_32px_-12px_rgba(0,0,0,0.5)] backdrop-blur-xl">
          ...
          >
            ...
            <Badge
              variant="outline"
              className="ml-1 h-5 min-w-5 justify-center rounded-full border-white/20 bg-white/10 px-1.5 text-[10px] font-semibold text-foreground group-data-[state=active]:border-white/40 group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white"
            >
              {liveExecs.length}
            </Badge>
            {liveErrors > 0 && (
              <Badge
                variant="outline"
                className="h-5 rounded-full border-red-500/40 bg-red-500/20 px-1.5 text-[10px] font-semibold text-red-300"
              >
                {liveErrors} erro{liveErrors === 1 ? "" : "s"}
              </Badge>
            )}
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="live" className="mt-0">
          <div className="rounded-2xl border border-border/60 bg-card/40 p-3 mb-4 text-xs text-muted-foreground">
            Aqui aparecem os fluxos disparados de verdade pelos seus contatos no WhatsApp. Clique em
            uma execução para ver o passo a passo, variáveis coletadas e eventuais erros.
          </div>
          <div className="grid gap-3 md:grid-cols-[340px_1fr]">
            {renderList(
              liveExecs,
              "Nenhum fluxo executado ainda. Assim que um contato acionar um fluxo, ele aparece aqui.",
            )}
            {renderDetail()}
          </div>
        </TabsContent>
          ...
          <div className="rounded-2xl border border-border/60 bg-card/40 p-3 mb-4 flex flex-wrap items-center gap-2">
            <Select value={pickedFlow} onValueChange={setPickedFlow}>
              <SelectTrigger className="w-72">
                <SelectValue placeholder="Escolha um fluxo para simular" />
              </SelectTrigger>
              <SelectContent>
                {flows.map((f) => (
            ...
            <span className="text-xs text-muted-foreground ml-auto">
              Teste o fluxo respondendo como se fosse o contato — nada é enviado no WhatsApp.
            </span>
          </div>
          <div className="grid gap-3 md:grid-cols-[340px_1fr]">
            {renderList(
              simExecs,
              "Nenhuma simulação ainda. Escolha um fluxo acima e clique em Iniciar simulação.",
            )}
            {renderDetail()}
          </div>
        </TabsContent>
        ...
        <TabsContent value="diag" className="mt-0">
          <div className="rounded-2xl border border-border/60 bg-card/40 p-3 mb-4 flex items-center gap-2">
            <span className="text-xs text-muted-foreground flex-1">
              Logs de correlação e runtime dos fluxos. Se uma mensagem chegou mas nenhuma execução
              foi criada, o motivo aparece aqui (ex.: sem fluxo ativo, fluxo sem START, nenhuma
              palavra-chave correspondeu, erro em bloco).
            </span>
            <Button
              variant="outline"
              size="sm"
            ...
            <div className="max-h-[70vh] overflow-y-auto divide-y divide-border/50">
              {!diag.length && (
                <div className="p-6 text-center text-xs text-muted-foreground">
                  Nenhum evento de diagnóstico ainda.
                </div>
              )}
              {diag.map((l) => {
                ...
                return (
                  <div
                    key={l.id}
                    className={`p-3 text-xs ${l.level === "error" ? "bg-red-500/5" : l.level === "warn" ? "bg-yellow-500/5" : ""}`}
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge
                        variant="outline"
                        className={`text-[9px] uppercase ${l.level === "error" ? "text-red-600 border-red-500/30" : l.level === "warn" ? "text-yellow-700 border-yellow-500/30" : ""}`}
                      >
                        {l.level}
                      </Badge>
                      {l.source && (
                        <code className="text-[10px] text-muted-foreground">{l.source}</code>
                      )}
                      {instance && (
                    ...
                    {Object.keys(meta).length > 0 && (
                      <pre className="mt-1 text-[10px] text-muted-foreground bg-background/5 rounded p-2 overflow-x-auto">
