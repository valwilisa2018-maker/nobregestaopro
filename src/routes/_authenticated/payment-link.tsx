...
const DEFAULT_PAGARME_RATES = [
  5.59, 8.59, 9.84, 11.09, 12.34, 13.59, 15.34, 16.59, 17.84, 19.09, 20.34, 21.59,
];
...
function PaymentLinkPage() {
  ...
  useEffect(() => {
    const loadRates = async () => {
      ...
      const [{ data }, { data: admin }] = await Promise.all([
        supabase
          .from("pagarme_installment_rates")
          .select("installments, fee_percent")
          .order("installments"),
  ...
  const saveRates = async () => {
    ...
    try {
      ...
      const rows = Array.from({ length: 12 }, (_, index) => ({
        ...
        fee_percent: Math.min(
          Math.max(Number(String(rates[index + 1]).replace(",", ".")) || 0, 0),
          99.999,
        ),
  ...
  const submit = async () => {
    ...
    const selected = (Object.keys(methods) as Array<keyof typeof methods>).filter(
      (k) => methods[k],
    );
  ...
  return (
    <div className="w-full max-w-none space-y-6">
      ...
      <Tabs defaultValue="payment" className="w-full space-y-4">
        <TabsList className="grid h-auto w-full max-w-xl grid-cols-2 rounded-xl border border-emerald-200 bg-emerald-50 p-1">
          <TabsTrigger
            value="payment"
            className="gap-2 rounded-lg py-2.5 data-[state=active]:bg-emerald-600 data-[state=active]:text-white"
          >
            <Link2 className="h-4 w-4" /> Gerar Pagamento
          </TabsTrigger>
          <TabsTrigger
            value="rates"
            className="gap-2 rounded-lg py-2.5 data-[state=active]:bg-emerald-600 data-[state=active]:text-white"
          >
            <Table2 className="h-4 w-4" /> Tabela de Taxas
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="payment" className="mt-0">
          ...
          <Card className="border-2" style={greenCardStyle}>
            <CardHeader className="pb-3">
              <CardTitle className="text-base" style={{ color: GREEN_DARK }}>
                Dados do pagamento
              </CardTitle>
            </CardHeader>
            <CardContent className="grid gap-6 lg:grid-cols-2 lg:items-stretch">
              ...
              <div
                className="h-full rounded-xl border p-5 space-y-4 lg:col-start-2 lg:row-start-1 lg:row-span-5"
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-semibold" style={{ color: GREEN_DARK }}>
                      Simular taxa do parcelamento
                    </div>
                    <p className="text-xs text-neutral-600">
                      Informe a taxa da operadora para calcular quanto cobrar do cliente e preservar
                      o valor líquido.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <span
                        className={feePayer === "customer" ? "text-white/80" : "text-neutral-500"}
                      >
                    ...
                    >
                      ...
                      <span
                        className={feePayer === "seller" ? "text-white/80" : "text-neutral-500"}
                      >
                ...
                {feePayer === "customer" ? (
                  ...
                  <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                    O valor do link permanece em {money(simulation.totalWithFee)} e a taxa estimada
                    de {money(simulation.feeAmount)} fica por conta da empresa.
                  </p>
                )}
                <p className="text-[11px] text-neutral-500">
                  Simulação estimada. Confirme a taxa vigente no seu contrato Pagar.me antes de
                  enviar a cobrança.
                </p>
              </div>
              ...
              {link && (
                ...
                >
                  <div className="text-xs font-semibold" style={{ color: GREEN_DARK }}>
                    Link gerado
                  </div>
          ...
          <Card className="border-2" style={greenCardStyle}>
            <CardHeader className="pb-3">
              ...
              <p className="text-xs text-neutral-500">
                Padrão oficial do Plano Parcelado Pagar.me. O simulador usa esses valores
                automaticamente.
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              {ratesLoading ? (
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                  {Array.from({ length: 12 }, (_, index) => index + 1).map((number) => (
                    ...
                    >
                      <Label className="font-semibold" style={{ color: GREEN_DARK }}>
                        {number}x
                      </Label>
                      <div className="relative mt-1.5">
                        <Input
                          ...
                          disabled={!isAdmin}
                          onChange={(event) =>
                            setRates((current) => ({ ...current, [number]: event.target.value }))
                          }
                        />
              ...
              {isAdmin ? (
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setRates(
                        Object.fromEntries(
                          DEFAULT_PAGARME_RATES.map((rate, index) => [index + 1, String(rate)]),
                        ),
                      )
                    }
                ...
                <p className="text-xs text-neutral-500">
                  Somente administradores podem alterar a tabela.
                </p>
              )}
              <p className="text-[11px] text-neutral-500">
                Referência: Plano Parcelado “Pronto para usar”, recebimento em 15 dias. Contratos
                Flex ou personalizados podem ter valores diferentes.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
