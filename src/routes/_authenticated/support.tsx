...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  ...
  in_analysis: {
    label: "Em análise",
    cls: "bg-violet-500/15 text-violet-500 border-violet-500/40",
  },
  ...
  waiting_dev: {
    ...
    cls: "bg-fuchsia-500/15 text-fuchsia-500 border-fuchsia-500/40",
  },
...
function Page() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      setUserId(u.id);
      const name =
        (u.user_metadata?.full_name as string | undefined) || u.email?.split("@")[0] || "Cliente";
    ...
    () =>
      tickets.filter((t) => {
        if (search) {
          ...
          if (!t.subject.toLowerCase().includes(q) && !String(t.ticket_number ?? "").includes(q))
            return false;
        }
        return true;
      }),
      ...
      {(contacts.phone || contacts.email || contacts.whatsapp) && (
        <div className="grid gap-3 sm:grid-cols-3">
          {contacts.whatsapp && (
            ...
            >
              <div className="grid h-11 w-11 place-items-center rounded-lg bg-emerald-500/20 text-emerald-500">
          ...
          {contacts.email && (
            ...
            >
              <div className="grid h-11 w-11 place-items-center rounded-lg bg-blue-500/20 text-blue-500">
          ...
          {loading ? (
            ...
            <div className="p-16 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Clique em <b>Abrir Novo Chamado</b> para falar com a nossa equipe.
              </p>
            </div>
          ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0">
                          <div className="text-[11px] font-mono text-muted-foreground">
                            #{String(t.ticket_number ?? "—").padStart(6, "0")}
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        {t.category && (
                          <span className="rounded-md bg-muted px-2 py-0.5">{t.category}</span>
                        )}
                        {p && (
                          <span className={`rounded-md border px-2 py-0.5 ${p.cls}`}>
                            {p.icon} {p.label}
                          </span>
                        )}
...
function Kpi({
  ...
  return (
    ...
    >
      ...
      <div className={`font-bold ${small ? "text-lg" : "text-2xl"} mt-1 text-foreground`}>
        {value}
      </div>
    </motion.div>
  );
...
function NewTicketDialog({
  ...
  const addFiles = (fs: FileList | File[] | null) => {
    ...
    if (list.length !== Array.from(fs).length)
      toast.warning("Alguns arquivos passam de 25 MB e foram ignorados");
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success(
        `Chamado #${String(ticket.ticket_number ?? "").padStart(6, "0")} criado com sucesso`,
      );
      onCreated(ticket.id);
      onOpenChange(false);
      reset();
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-primary" /> Abrir Novo Chamado
          </DialogTitle>
          <DialogDescription>
            Explique com detalhes o que aconteceu. Quanto mais informação, mais rápido resolvemos.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Imagens, vídeos, áudios, PDFs — até 25 MB cada
              </div>
              <input
                ref={fileRef}
                type="file"
          ...
          <label className="flex items-start gap-2 text-sm cursor-pointer">
            ...
            <span className="text-muted-foreground">
              Autorizo a equipe técnica a acessar informações deste chamado para análise.
            </span>
          </label>
        </div>
        <DialogFooter>
        ...
        <SheetContent side="right" className="w-full sm:max-w-2xl p-0 flex flex-col">
          <SheetHeader className="p-4 border-b bg-gradient-to-br from-primary/10 to-transparent">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-[11px] font-mono text-muted-foreground">
                  Chamado #{String(ticket.ticket_number ?? "—").padStart(6, "0")}
                </div>
                ...
                <div className="text-xs text-muted-foreground mt-2 flex flex-wrap gap-x-4 gap-y-0.5">
                  ...
                  {ticket.environment && (
                    <span>
                      {ticket.environment} · {ticket.browser}
                    </span>
                  )}
                </div>
              </div>
            ...
            {messages.map((m) => {
              ...
              return (
                <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                  ...
                  >
                    {!mine && (
                      <div className="text-[10px] font-semibold opacity-80 mb-0.5">Suporte</div>
                    )}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {replyFiles.map((f, i) => (
                    ...
                    >
                      ...
                      {f.name}
                      <button
                        onClick={() => setReplyFiles((prev) => prev.filter((_, k) => k !== i))}
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </div>
      ...
      <Dialog open={rateOpen} onOpenChange={setRateOpen}>
        <DialogContent>
            ...
            <div className="flex justify-center gap-1">
              {[1, 2, 3, 4, 5].map((n) => (
                <button key={n} onClick={() => setRating(n)}>
                  <Star
                    className={`h-9 w-9 transition ${n <= rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`}
                  />
                </button>
              ))}
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={rate} className="gap-2">
