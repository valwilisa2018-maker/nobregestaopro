...
import { AlertTriangle, Check, CircleAlert, MessagesSquare, RefreshCw, Workflow } from "lucide-react";
...
function Page() {
  ...
  const startFlow = useServerFn(startFlowForConversation);
            ...
            <>
              <div className="border-b border-border/60 px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <Workflow className="h-4 w-4" /> Iniciar fluxo
                  </Button>
          ...
          <div className="max-h-[50vh] overflow-y-auto divide-y divide-border/50">
            {flows.map((f) => (
              <div key={f.id} className="flex items-center justify-between gap-2 py-2">
                ...
                <Button
                  size="sm"
                  disabled={!selected || startingId === f.id}
                  onClick={async () => {
                    if (!selected) return;
                    setStartingId(f.id);
                    try {
                      await startFlow({ data: { conversationId: selected.id, flowId: f.id } });
            ...
            {!flows.length && <div className="p-6 text-center text-xs text-muted-foreground">Nenhum fluxo cadastrado</div>}
          </div>
        </DialogContent>
      </Dialog>
    </PageShell>
