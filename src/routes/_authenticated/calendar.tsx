...
function CalendarPage() {
  ...
  const visible = useMemo(
    () =>
      events.filter(
        (e) =>
          enabledCals[e.calendar] &&
          (activeConn === "all" || (e.connectionId ?? null) === activeConn),
      ),
  ...
  function saveEvent(ev: EventItem) {
    ...
    if (+new Date(ev.end) <= +new Date(ev.start))
      return toast.error("Hora final deve ser após a inicial");
    ...
    <aside className="w-72 shrink-0 border-r bg-background p-4 flex flex-col gap-4">
      ...
      <div className="space-y-1">
        ...
        <div className="text-[10px] text-muted-foreground">
          Cada instância tem sua própria agenda.
        </div>
      </div>
      ...
      <Sheet>
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 border-b flex items-center gap-2 px-3">
            ...
            <Button variant="ghost" size="icon" onClick={() => setCursor(shift(cursor, view, -1))}>
              ...
              <Tabs value={view} className="h-full">
                <TabsContent value="day" className="h-full m-0">
                  <TimeGrid
                    ...
                    events={visible}
                    onSlot={(d) =>
                      openCreate({ start: d.toISOString(), end: addMinutes(d, 60).toISOString() })
                    }
                    onOpen={openEdit}
                  />
                </TabsContent>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} className="py-2 text-center border-l">
                ...
                <div className={cn("text-xl font-semibold", today && "text-primary")}>
                  {format(d, "d")}
                </div>
              </div>
            );
          ...
          <div>
            {hours.map((h) => (
              <div
                key={h}
                style={{ height: HOUR_H }}
                className="text-[10px] text-muted-foreground text-right pr-1 -translate-y-2"
              >
        ...
        {days.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                "border-r border-b p-1 flex flex-col gap-0.5 hover:bg-accent/30 transition-colors cursor-pointer overflow-hidden",
                dim && "bg-muted/30",
              )}
              onClick={() => onDay(d)}
            >
        ...
        {Array.from(groups.entries()).map(([k, list]) => (
          <div key={k}>
            <div className="text-sm font-semibold mb-2 capitalize">
              {format(new Date(k), "EEEE, d 'de' MMMM", { locale: ptBR })}
            </div>
            <div className="space-y-1">
              {list.map((e) => {
                const meta = CAL_META[e.calendar];
                return (
                  <button
                    key={e.id}
                    onClick={() => onOpen(e)}
                    className="w-full text-left p-3 rounded-lg border hover:bg-accent/40 flex items-center gap-3"
                  >
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {draft.createdByAi && (
              <span className="ml-2 text-xs inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-600">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {(Object.keys(CAL_META) as CalId[]).map((k) => (
                <button
                  key={k}
                  onClick={() => setDraft({ ...draft, calendar: k })}
                  className={cn(
                    "flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm transition-colors",
                    draft.calendar === k ? "border-transparent text-foreground" : "hover:bg-accent",
                  )}
          ...
          <div>
            <Label className="flex items-center gap-1">
              <UsersIcon className="h-3 w-3" /> Convidados
            </Label>
            <Input
              value={draft.guests ?? ""}
...
function AiChat({
  ...
  onPropose: (e: Partial<EventItem>) => void;
  events: EventItem[];
}) {
  ...
  const [msgs, setMsgs] = useState<ChatMsg[]>([
    {
      id: uid(),
      role: "ai",
      text: 'Olá! Diga algo como: "Agendar reunião com Pedro quarta às 15h por 1h".',
    },
  ]);
      ...
      if (!parsed) {
        setMsgs((m) => [
          ...m,
          {
            id: uid(),
            role: "ai",
            text: 'Não entendi a data/hora. Tente: "reunião amanhã às 14h por 30min".',
          },
        ]);
        return;
      }
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-40 w-[360px] max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-8rem)] rounded-2xl border bg-background shadow-2xl flex flex-col overflow-hidden">
          <div className="p-3 border-b flex items-center gap-2 bg-gradient-to-r from-purple-500/10 to-blue-500/10">
            <div
              className="h-8 w-8 rounded-full flex items-center justify-center text-foreground"
              style={{ background: "linear-gradient(135deg,#8b5cf6,#3b82f6)" }}
            >
            ...
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Interpreta comandos e agenda por você
              </div>
            </div>
            <Button
              variant="ghost"
                    ...
                    {m.proposal && (
                      <div className="mt-2 rounded-lg border bg-background text-foreground p-2">
                        ...
                        <div className="text-xs mt-1">
                          <b>{m.proposal.title}</b>
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {format(new Date(m.proposal.start), "EEE dd/MM HH:mm", { locale: ptBR })}{" "}
                          – {format(new Date(m.proposal.end), "HH:mm")}
                        </div>
                        <div className="flex gap-2 mt-2">
                          <Button size="sm" className="h-7" onClick={() => onPropose(m.proposal!)}>
                            Abrir e revisar
                          </Button>
                        </div>
                      </div>
...
function parseCommand(text: string): EventItem | null {
  ...
  const timeM =
    lower.match(/(\d{1,2})(?::(\d{2}))?\s*h(?:oras?)?/) || lower.match(/(\d{1,2}):(\d{2})/);
  ...
  title = title
    .replace(
      /\b(hoje|amanhã|amanha|segunda|terça|terca|quarta|quinta|sexta|s[áa]bado|domingo)\b/gi,
      "",
    )
    .trim();
  ...
  return {
    ...
    calendar: "ai",
    createdByAi: true,
    description: `Criado por IA a partir de: "${text}"`,
  };
}
