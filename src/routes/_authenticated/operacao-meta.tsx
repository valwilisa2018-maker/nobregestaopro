...
import {
  ...
  Flame, TrendingUp, Calendar, FileText, Sun, Moon, BarChart3,
  LayoutGrid, PartyPopper, LineChart as LineIcon, Award, Copy, Printer,
  Zap, Star,
} from "lucide-react";
...
type MenuKey = "diaria" | "mensal" | "dinamica" | "tendencias" | "produtores" | "conquistas" | "relatorios";

const MENU: { key: MenuKey; label: string; icon: any }[] = [
  { key: "diaria", label: "Análise Diária", icon: LayoutGrid },
  { key: "mensal", label: "Análise Mensal", icon: BarChart3 },
  { key: "dinamica", label: "Dinâmica", icon: PartyPopper },
  { key: "tendencias", label: "Tendências", icon: LineIcon },
...
function OperacaoMetaPage() {
  ...
  const [active, setActive] = useState<MenuKey>("diaria");

  // Default to dark on first visit to this page
  useEffect(() => {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4">
        ...
        <div className="min-w-0">
          {active === "diaria" && <DiariaView delivered={delivered} producers={producers.data ?? []} computePts={computePts} catName={catName} sumPts={sumPts} qc={qc} />}
          {active === "mensal" && <MensalView delivered={delivered} producers={producers.data ?? []} computePts={computePts} catName={catName} sumPts={sumPts} prodOf={prodOf} />}
          {active === "dinamica" && <DinamicaView delivered={delivered} producers={producers.data ?? []} computePts={computePts} sumPts={sumPts} prodOf={prodOf} />}
          {active === "tendencias" && <TendenciasView delivered={delivered} producers={producers.data ?? []} computePts={computePts} sumPts={sumPts} />}
          {active === "produtores" && <ProdutoresView delivered={delivered} producers={producers.data ?? []} computePts={computePts} />}
          {active === "conquistas" && <ConquistasView delivered={delivered} producers={producers.data ?? []} computePts={computePts} catName={catName} prodOf={prodOf} />}
          {active === "relatorios" && <RelatoriosView delivered={delivered} producers={producers.data ?? []} computePts={computePts} sumPts={sumPts} />}
        </div>
      </div>
    </div>
  );
...
/* ============================ ANÁLISE DIÁRIA ============================ */
function DiariaView({ delivered, producers, computePts, catName, sumPts, qc }: any) {
...
/* ============================ ANÁLISE MENSAL ============================ */
function MensalView({ delivered, producers, computePts, catName, sumPts, prodOf }: any) {
  ...
  const reBest = altPerProducer[0]; // most reworks
  const finalizador = useMemo(() => {
    // producer with most projects in last day
...
/* ============================ DINÂMICA ============================ */
function DinamicaView({ delivered, producers, computePts, sumPts, prodOf }: any) {
  ...
  // Goal: sum of daily goals * working days
...
/* ============================ TENDÊNCIAS ============================ */
function TendenciasView({ delivered, producers, computePts, sumPts }: any) {
  // Last 30 days series
...
/* ============================ PRODUTORES ============================ */
function ProdutoresView({ delivered, producers, computePts }: any) {
...
/* ============================ CONQUISTAS ============================ */
function ConquistasView({ delivered, producers, computePts, catName, prodOf }: any) {
...
/* ============================ RELATÓRIOS ============================ */
function RelatoriosView({ delivered, producers, computePts, sumPts }: any) {
