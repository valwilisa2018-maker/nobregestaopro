...
> = {
  START: {
    label: "Gatilho",
    sub: "Inicia o fluxo",
    hint: "Ponto de partida do fluxo. Define quando ele começa.",
  ...
  MESSAGE: {
    label: "Mensagem",
    sub: "Enviar texto",
    hint: "Envia uma mensagem de texto ao contato.",
  ...
  CONDITION: {
    label: "Condição",
    sub: "Dividir o fluxo",
    hint: "Divide o fluxo em dois caminhos com base em uma condição.",
  ...
  WAIT: {
    label: "Aguardar",
    sub: "Tempo de espera",
    hint: "Aguarda um tempo antes de seguir para o próximo bloco.",
  ...
  VIDEO: {
    label: "Vídeo",
    sub: "Enviar vídeo",
    hint: "Envia um arquivo de vídeo para o contato (upload).",
  ...
  IMAGE: {
    label: "Imagem",
    sub: "Enviar imagem",
    hint: "Envia uma imagem para o contato (upload).",
  ...
  AUDIO: {
    label: "Áudio",
    sub: "Enviar áudio",
    hint: "Envia um áudio para o contato (upload).",
  ...
  TYPING: {
    label: "Digitando",
    sub: "Simular digitação",
    hint: "Mostra 'digitando...' para simular uma resposta humana.",
  ...
  RECORDING: {
    label: "Gravando",
    sub: "Simular gravação",
    hint: "Mostra 'gravando áudio...' antes de enviar.",
  ...
  TAGS: {
    label: "Etiquetas",
    sub: "Adicionar etiquetas",
    hint: "Adiciona etiquetas ao contato para segmentação.",
  ...
  QUESTION: {
    label: "Pergunta",
    sub: "Aguardar resposta",
    hint: "Envia uma pergunta e aguarda o contato responder.",
  ...
  CAPTURE_NAME: {
    label: "Capturar Nome",
    sub: "Pergunta e salva o nome",
    hint: "Pergunta o nome do contato e salva na variável nome.",
  ...
  SCHEDULE: {
    label: "Agendamento",
    sub: "Agendar reunião",
    hint: "Envia link/instrução de agendamento.",
  ...
  BROADCAST: {
    label: "Disparo",
    sub: "Disparo em massa",
    hint: "Envia mensagem para vários contatos.",
  ...
  YESNO: {
    label: "Sim / Não",
    sub: "Dividir por resposta",
    hint: "Divide o fluxo com base em resposta Sim ou Não.",
    icon: <GitBranch className="h-3.5 w-3.5" />,
    color: "bg-amber-600",
    outputs: [
      { id: "sim", label: "Sim" },
  ...
  WEBHOOK: {
    label: "Webhook",
    sub: "Chamar API externa",
    hint: "Faz uma requisição HTTP para um serviço externo.",
  ...
  HANDOFF: {
    label: "Atendente",
    sub: "Transferir para humano",
    hint: "Transfere a conversa para um atendente humano.",
  ...
  SEQUENCE: {
    label: "Sequência",
    sub: "Etiqueta + inscrever em fluxo",
    hint: "Adiciona etiqueta ao contato e o inscreve em outro fluxo. Ao terminar este, o fluxo alvo é iniciado.",
...
async function uploadMediaFast(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = window.setTimeout(() => {
      xhr.abort();
      reject(
        new Error("O upload demorou demais. Reduza o vídeo para menos de 10 MB e tente novamente."),
      );
...
function BlockNode({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`rounded-lg border bg-card shadow-md w-[220px] max-w-[220px] transition ${selected ? "ring-2 ring-primary" : ""} ${invalid ? "border-destructive ring-2 ring-destructive animate-pulse" : "border-emerald-500/40"}`}
    >
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 text-[10px] uppercase tracking-wide text-muted-foreground border-b border-border/50">
        <span
          className={`h-4 w-4 rounded flex items-center justify-center text-foreground ${meta.color}`}
        >
          {meta.icon}
        </span>
      ...
      {meta.outputs.length === 1 && (
        <Handle type="source" position={Position.Right} id={meta.outputs[0].id} />
      )}
      {meta.outputs.length > 1 &&
        meta.outputs.map((o, i) => (
          <div key={o.id} className="relative">
            <div className="text-[10px] text-muted-foreground px-3 py-0.5 text-right pr-6">
              {o.label}
            </div>
            <Handle
              type="source"
...
const TEMPLATES: Array<{ name: string; nodes: Node<BlockData>[]; edges: Edge[] }> = [
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        data: { kind: "MESSAGE", label: "Mensagem", text: "Olá! Bem-vindo 👋" },
      },
      {
        id: "q",
        type: "block",
        position: { x: 500, y: 120 },
        data: { kind: "YESNO", label: "Sim/Não", text: "Quer falar com atendente?" },
      },
      {
        id: "h",
        type: "block",
        position: { x: 760, y: 40 },
        data: { kind: "HANDOFF", label: "Atendente" },
      },
  ...
  },
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        data: { kind: "QUESTION", label: "Pergunta", text: "Qual seu nome?", variable: "nome" },
      },
      {
        id: "c",
        type: "block",
        position: { x: 460, y: 120 },
        data: { kind: "QUESTION", label: "Pergunta", text: "Cidade?", variable: "cidade" },
      },
      {
        id: "w",
        type: "block",
        position: { x: 680, y: 120 },
        data: { kind: "WEBHOOK", label: "Webhook", url: "https://exemplo.com/lead" },
      },
      { id: "e", type: "block", position: { x: 900, y: 120 }, data: { kind: "END", label: "Fim" } },
    ],
    edges: [
      { id: "e1", source: "s", target: "n" },
      { id: "e2", source: "n", target: "c" },
      { id: "e3", source: "c", target: "w" },
...
function Page() {
  return (
    ...
    >
      <div className="mb-4">
        <TutorialVideo moduleKey="modulo_03" title="Como criar Fluxos" />
      </div>
      <ReactFlowProvider>
        <Builder />
      </ReactFlowProvider>
...
function Builder() {
  ...
  const saveM = useMutation({
    mutationFn: async () => {
      ...
      setNodes((ns) =>
        ns.map((n) => ({ ...n, data: { ...n.data, _invalid: badIds.has(n.id) } as BlockData })),
      );
      if (badIds.size > 0) {
        // Auto-clear after 4s
        setTimeout(() => {
          setNodes((ns) =>
            ns.map((n) => ({ ...n, data: { ...n.data, _invalid: false } as BlockData })),
          );
        }, 4000);
        const names = nodes
          .filter((n) => badIds.has(n.id))
          .map((n) => n.data.label)
          .slice(0, 3)
          .join(", ");
        throw new Error(
          `Existem ${badIds.size} bloco(s) desconectado(s): ${names}${badIds.size > 3 ? "…" : ""}. Conecte todos os blocos antes de salvar.`,
        );
      }
      if (!nodes.some((n) => n.data.kind === "START"))
        throw new Error("O fluxo precisa de um bloco Gatilho (START).");
      if (!nodes.some((n) => n.data.kind === "END"))
        throw new Error("O fluxo precisa de pelo menos um bloco Fim (END).");
      return save({
        data: {
          id: flowId,
          name,
  ...
  const genM = useMutation({
    ...
    onSuccess: (res) => {
      try {
        const parsed = JSON.parse(res.flowJson) as {
          nodes?: Array<{
            id: string;
            type: string;
            message?: string;
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          <Dialog open={aiOpen} onOpenChange={setAiOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm">
                <Bot className="h-4 w-4" /> Gerar IA
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
              ...
              <Textarea
                ...
                placeholder="Ex.: Qualifica lead de imóvel, pede nome/cidade e envia para o corretor"
              />
          ...
          >
            <SelectTrigger className="h-9 w-[220px]">
              <SelectValue placeholder="Fluxos salvos" />
            </SelectTrigger>
            <SelectContent>
          ...
          <Button
            ...
            size="sm"
            disabled={!flowId}
            onClick={() => flowId && delM.mutate(flowId)}
          >
                ...
                return (
                  <Tooltip key={k}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <span
                          className={`h-6 w-6 rounded flex items-center justify-center text-foreground shrink-0 ${m.color}`}
                        >
                          {m.icon}
                        </span>
              ...
              <>
                ...
                {selected.data.kind === "CONDITION" && (
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="ex.: {{cidade}} == 'SP'"
                    />
                    <p className="text-[10px] text-muted-foreground">
                      Saídas: <b>Verdadeiro</b> quando a condição for atendida, <b>Falso</b> caso
                      contrário.
                    </p>
                  </div>
                )}
                {selected.data.kind === "SEQUENCE" && (
                  <div className="space-y-1">
                    ...
                    />
                    <p className="text-[10px] text-muted-foreground">
                      Ao passar por este bloco, o contato recebe as etiquetas acima e é inscrito no
                      fluxo cujo gatilho corresponde. Esse fluxo inicia após o atual encerrar.
                    </p>
                  </div>
                )}
              ...
              <Card key={f.id} className={flowId === f.id ? "ring-2 ring-primary" : ""}>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="line-clamp-1 text-xs">
                    {f.trigger ?? "sem gatilho"}
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex gap-1">
                  <Button size="sm" variant="outline" onClick={() => openFlow(f)}>
                    Abrir
                  </Button>
                  <Button
                    size="sm"
