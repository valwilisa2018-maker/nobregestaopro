...
const formatTokens = (n: number) => {
  if (n >= 1_000_000)
    return `${(n / 1_000_000).toLocaleString("pt-BR", { maximumFractionDigits: 1 })} mi de créditos`;
  ...
  useEffect(() => {
    (async () => {
      ...
      setPlans(
        (data ?? []).map((p) => ({
          ...(p as Plan),
          features: Array.isArray((p as { features: unknown }).features)
            ? (p as Plan).features
            : [],
        })),
      );
      ...
      <div className="flex justify-center mb-8">
        <div className="flex items-center gap-1 bg-card/60 border border-border p-1 rounded-full">
          ...
          >
            Anual
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full border ${cycle === "annual" ? "bg-white/15 text-foreground border-border" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"}`}
            >
              -2 meses
            </span>
          </button>
      ...
      {loading ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4 pt-6 items-stretch">
          {plans.map((p) => {
            ...
            const savingsPct =
              showAnnual && annualFromMonthly > 0
                ? Math.round((savings / annualFromMonthly) * 100)
                : 0;
            ...
            return (
              ...
              >
                ...
                <div className="pointer-events-none absolute inset-0 rounded-2xl overflow-hidden">
                  <div
                    className={`absolute inset-x-0 -top-24 h-40 opacity-60 blur-2xl ${p.highlight ? "bg-emerald-500/20" : "bg-primary/10"}`}
                  />
                </div>
                {p.highlight && (
                ...
                <div className="relative flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      return (
                        <div
                          className={`p-1.5 rounded-lg ${p.highlight ? "bg-emerald-500/15 text-emerald-400" : "bg-primary/10 text-primary"}`}
                        >
                    ...
                    <h3
                      className={`text-base font-bold ${p.highlight ? "text-foreground" : "text-foreground/80"}`}
                    >
                      {p.name}
                    </h3>
                  </div>
                ...
                <ul className="relative mt-4 space-y-2 flex-grow">
                  {p.features.map((f, i) => (
                    <li
                      key={i}
                      className={`flex items-start gap-2 text-xs ${p.highlight ? "text-foreground/90" : "text-muted-foreground"}`}
                    >
                      <Check
                        className={`w-4 h-4 shrink-0 mt-0.5 ${p.highlight ? "text-emerald-400" : "text-primary/80"}`}
                        strokeWidth={2.5}
                      />
                      <span>{f}</span>
                    </li>
