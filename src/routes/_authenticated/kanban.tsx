...
function CardFolderActions({ cardId }: { cardId: string }) {
  ...
  const allFolders = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("project_folders" as any)
        .select("id, folder_name, client_name")
        .is("kanban_card_id", null)
        .order("created_at", { ascending: false });
      return (data ?? []) as any[];
    },
  });
  async function linkFolder(folderId: string) {
    const { error } = await supabase
      .from("project_folders" as any)
      .update({ kanban_card_id: cardId })
      .eq("id", folderId);
    if (error) { toast.error(error.message); return; }
    toast.success("Pasta vinculada");
    setPickOpen(false); q.refetch();
  }
  async function createAndLink() {
    ...
    const { error } = await supabase
      .from("project_folders" as any)
      .insert({ folder_name: name, kanban_card_id: cardId, created_by: u.user?.id });
    if (error) { toast.error(error.message); return; }
    toast.success("Pasta criada e vinculada"); q.refetch();
  }
  async function unlink(e: React.MouseEvent) {
    e.stopPropagation();
    if (!f) return;
    if (!window.confirm("Desvincular pasta deste card?")) return;
    const { error } = await supabase
      .from("project_folders" as any)
      .update({ kanban_card_id: null })
      .eq("id", f.id);
    if (error) { toast.error(error.message); return; }
    q.refetch();
  }
    ...
    <div className="flex flex-wrap items-center gap-1 pt-1 border-t border-dashed" onClick={(e) => e.stopPropagation()}>
      {f ? (
        <>
          ...
          <button onClick={unlink} className="text-[10px] px-2 py-0.5 rounded bg-muted hover:bg-muted/80">Desvincular</button>
        </>
      ) : (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); setPickOpen(true); }}
            className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-primary/10 text-primary hover:bg-primary/20"
          >
            <FolderOpen className="w-3 h-3" /> Vincular pasta
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); createAndLink(); }}
            className="text-[10px] px-2 py-0.5 rounded bg-muted hover:bg-muted/80"
          >
            + Nova
          </button>
        </>
      ...
      <Dialog open={pickOpen} onOpenChange={setPickOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Vincular pasta existente</DialogTitle></DialogHeader>
          <div className="max-h-80 overflow-y-auto space-y-1">
            {allFolders.isLoading ? (
              <div className="text-sm text-muted-foreground">Carregando...</div>
            ) : (allFolders.data ?? []).length === 0 ? (
              <div className="text-sm text-muted-foreground">Nenhuma pasta livre. Crie uma nova.</div>
            ) : (
              (allFolders.data ?? []).map((fo: any) => (
                <button
                  key={fo.id}
                  onClick={() => linkFolder(fo.id)}
                  className="w-full text-left p-2 rounded hover:bg-muted border"
                >
                  <div className="text-sm font-medium">{fo.folder_name}</div>
                  {fo.client_name && <div className="text-xs text-muted-foreground">{fo.client_name}</div>}
                </button>
              ))
            )}
          </div>
