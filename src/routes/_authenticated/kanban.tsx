...
const KANBAN_CARDS_SELECT =
  "*, producer:producers!service_orders_producer_id_fkey(name,avatar_url), sales:sales!service_orders_sale_id_fkey(total_amount, paid_amount, payment_status, ticket_type, trello_link, google_drive_link, platform_link, producer_id, seller_name_snapshot, producer_name_snapshot, expected_delivery_date, service_quantity, video_duration_seconds, video_duration_breakdown_seconds, customers(name,company,phone), sellers(name,avatar_url), producers(name,avatar_url))";
...
function KanbanPage() {
  ...
  const move = async (cardId: string, columnId: string) => {
    ...
    const currentCard = qc.getQueryData<any[]>(["kanban-cards"])?.find((c: any) => c.id === cardId);
    const currentCol = cols.data?.find((c: any) => c.id === currentCard?.column_id);
    const isNewDelivery = col?.is_done === true && currentCol?.is_done === false;
      ...
      (old ?? []).map((c: any) =>
        c.id === cardId
          ? {
              ...
              column_id: columnId,
              sort_order: newSort,
              delivered_at: isNewDelivery ? nowIso : c.delivered_at,
            }
          : c,
      ),
    );
    const { error } = await supabase
      ...
      .update({
        column_id: columnId,
        sort_order: newSort,
        ...(isNewDelivery ? { delivered_at: nowIso } : {}),
      })
      .eq("id", cardId);
    if (error) {
  ...
  const moveMany = async (cardIds: string[], columnId: string) => {
    ...
    const prev = qc.getQueryData<any[]>(["kanban-cards"]);
    const newlyDeliveredIds = new Set(
      (prev ?? [])
        .filter((c: any) => idSet.has(c.id))
        .filter((c: any) => {
          const currentCol = cols.data?.find((column: any) => column.id === c.column_id);
          return col?.is_done === true && currentCol?.is_done === false;
        })
        .map((c: any) => c.id),
    );
    qc.setQueryData<any[]>(["kanban-cards"], (old) =>
      (old ?? []).map((c: any) =>
        idSet.has(c.id)
          ? {
              ...
              delivered_at: newlyDeliveredIds.has(c.id) ? nowIso : c.delivered_at,
            }
          : c,
      ),
    );
      ...
      cardIds.map((id, i) =>
        supabase
          ...
          .update({
            ...
            ...(newlyDeliveredIds.has(id) ? { delivered_at: nowIso } : {}),
          })
          .eq("id", id),
      ),
    );
