    ...
    <PageShell>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#0b1120] via-[#0f172a] to-[#1e1b4b] p-6 md:p-8 shadow-premium">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-sm text-slate-300/80 max-w-2xl">
                Configure a WhatsApp Cloud API oficial da Meta — templates aprovados, janela de 24h
                e disparo em massa com alta entregabilidade.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className={`h-9 px-3 ${statusOk ? "bg-emerald-500/10 text-emerald-300 border-emerald-400/40" : "bg-white/5 text-slate-300 border-white/10"}`}
            >
              {statusOk ? (
            ...
            <Button
              ...
              className="h-9 bg-white/10 hover:bg-white/15 text-white border border-white/15 backdrop-blur"
            >
              {busy === "verify" ? (
      ...
      <Tabs defaultValue="credentials" className="space-y-4 mt-4">
        <TabsList className="bg-card/40 border border-border/60 p-1 rounded-xl">
          <TabsTrigger
            value="credentials"
            className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-indigo-600 data-[state=active]:text-white rounded-lg gap-1.5"
          >
        ...
        <TabsContent value="credentials">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Cole os dados obtidos no Meta Business Manager → WhatsApp → Configuração da API.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <Field
                ...
                onChange={(v) => setCfg({ ...cfg, phone_number_id: v })}
              ...
              <Field
                ...
                onChange={(v) => setCfg({ ...cfg, business_account_id: v })}
              />
              <Field
                label="App ID"
                value={cfg.app_id ?? ""}
                onChange={(v) => setCfg({ ...cfg, app_id: v })}
              />
              <Field
                label="App Secret"
                type="password"
                value={cfg.app_secret ?? ""}
                onChange={(v) => setCfg({ ...cfg, app_secret: v })}
              />
              <Field
                ...
                onChange={(v) => setCfg({ ...cfg, access_token: v })}
              />
              <div className="space-y-2">
                <Label>Versão da Graph API</Label>
                <Select
                  value={cfg.graph_version}
                  onValueChange={(v) => setCfg({ ...cfg, graph_version: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex items-center gap-3 pt-6">
                <Switch
                  ...
                  onCheckedChange={(v) => setCfg({ ...cfg, is_default: v })}
                />
        ...
        <TabsContent value="webhook">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Cadastre este endpoint em WhatsApp → Configuração → Webhooks, campos <b>messages</b>{" "}
                e <b>message_template_status_update</b>.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => {
                      navigator.clipboard.writeText(cfg.webhook_verify_token ?? "");
                      toast.success("Copiado");
                    }}
                  >
                    <Copy className="h-4 w-4" />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCfg({
                        ...cfg,
                        webhook_verify_token: crypto.randomUUID().replace(/-/g, ""),
                      })
                    }
                  >
              ...
              <div className="rounded-lg border border-border/60 bg-muted/30 p-3 text-xs text-muted-foreground flex gap-2">
                ...
                <span>
                  A Meta valida o webhook via GET com <code>hub.challenge</code>. Cole a URL e o
                  Verify Token exatamente iguais no painel da Meta.
                </span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Templates aprovados pela Meta são obrigatórios para iniciar conversas fora da
                  janela de 24h.
                </CardDescription>
              </div>
              <div className="flex gap-2">
                ...
                <NewTemplateDialog
                  onCreate={async (payload) => {
                    ...
                    if (r.ok) {
                      toast.success(`Enviado para aprovação (${r.status})`);
                      load();
                      return true;
                    }
                  ...
                  {templates.map((t) => (
                    <div key={t.id} className="flex items-center gap-3 py-3">
                      <div className="flex-1">
                        ...
                        {t.rejection_reason && (
                          <div className="text-xs text-red-500 mt-1">{t.rejection_reason}</div>
                        )}
                      </div>
                      <Button
                        size="icon"
          ...
          <TestSend
            ...
            onSendText={async (to, text) => {
              ...
              if (r.ok) toast.success(`Enviado ${r.messageId ?? ""}`);
                ...
                toast.error(`${r.error ?? "falhou"}${"hint" in r && r.hint ? ` — ${r.hint}` : ""}`);
            }}
          />
        </TabsContent>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-300/80">
                    Siga cada etapa na ordem para conectar a API oficial da Meta.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-amber-200/80 leading-relaxed">
                    Após o cliente enviar uma mensagem, você tem 24h para responder livremente com
                    texto ou mídia. Passado esse tempo, é obrigatório usar um{" "}
                    <span className="font-semibold text-amber-100">template aprovado</span> para
                    reabrir a conversa. O sistema usa templates automaticamente no Disparo em Massa
                    quando necessário.
                  </p>
                </div>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={`text-xs ${map[status] ?? map.LOCAL}`}>
      {status}
    </Badge>
  );
}
...
function NewTemplateDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Criar template para aprovação</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3">
          <Field
            label="Nome (a-z, 0-9, _)"
            required
            value={form.name}
            onChange={(v) =>
              setForm({ ...form, name: v.toLowerCase().replace(/[^a-z0-9_]/g, "_") })
            }
            placeholder="promo_natal_2026"
          />
          ...
          <Field
            ...
            onChange={(v) => setForm({ ...form, headerText: v })}
          />
          <div className="space-y-2">
            <Label>
...
function TestSend({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>Enviar template (fora da janela)</CardTitle>
          <CardDescription>Usa a HSM aprovada — ideal para disparos frios.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Select value={tplId} onValueChange={setTplId}>
              <SelectTrigger>
                <SelectValue placeholder={templates.length ? "Selecione" : "Nenhum aprovado"} />
              </SelectTrigger>
              <SelectContent>
                {templates.map((t) => (
          ...
          <Field
            label="Variáveis do corpo (separadas por |)"
            value={params}
            onChange={setParams}
            placeholder="João|10%|amanhã"
          />
          <Button
            className="w-full"
            disabled={!to || !tpl || busy}
            onClick={async () => {
              setBusy(true);
      ...
      <Card>
        <CardHeader>
          <CardTitle>Enviar texto livre (janela 24h)</CardTitle>
          <CardDescription>Só funciona se o cliente enviou algo nas últimas 24h.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <Field label="Número (E.164)" value={to} onChange={setTo} placeholder="5511999998888" />
          <div className="space-y-2">
          ...
          <Button
            className="w-full"
            variant="outline"
            disabled={!to || !text || busy}
            onClick={async () => {
              setBusy(true);
              await onSendText(to, text);
              setBusy(false);
            }}
          >
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{" "}
