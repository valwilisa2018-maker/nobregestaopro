...
function ProducersPage() {
  ...
  const toggleActive = async (id: string, active: boolean) => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(active ? "Produtor ativado" : "Produtor desativado");
      qc.invalidateQueries();
    }
  };
  ...
  const saveConfig = async () => {
    ...
    const { error } = await supabase
      .from("producers")
      .update({ custom_kanban_columns: customCols })
      .eq("id", configProducer.id);
    if (error) toast.error(error.message);
    else {
      toast.success("Configuração salva");
      setConfigProducer(null);
      qc.invalidateQueries();
    }
  };
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    {editing.avatar_url && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditing({ ...editing, avatar_url: null })}
                      >
                        Remover
                      </Button>
                    )}
      ...
      <Dialog open={!!configProducer} onOpenChange={(o) => !o && setConfigProducer(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Configurar Kanban - {configProducer?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <p className="text-sm text-muted-foreground">
              Personalize os nomes das colunas do Kanban para este produtor (máximo 5 colunas).
            </p>
                ...
                <div className="grid grid-cols-2 gap-2 py-2 border-y text-sm">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="font-medium">
                      {Number(p.average_delivery_days ?? 0).toFixed(1)} dias
                    </p>
                  </div>
                </div>
