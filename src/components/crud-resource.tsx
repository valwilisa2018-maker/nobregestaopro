...
export function CrudResource({
  table, title, description, icon, singular, fields,
  orderBy = "created_at", ascending = false, renderRow,
}: CrudResourceProps) {
  const { user } = useAuth();
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [user, table]);
          ...
          <div className="space-y-4 py-2 max-h-[60vh] overflow-y-auto pr-1">
            {fields.map((f) => (
              <div key={f.name} className="space-y-2">
                ...
                {f.type === "textarea" ? (
                  <Textarea id={f.name} value={(form[f.name] as string) ?? ""} placeholder={f.placeholder}
                    onChange={(e) => setForm((s) => ({ ...s, [f.name]: e.target.value }))} rows={4} />
                ) : f.type === "boolean" ? (
                  <div className="flex items-center gap-2">
                    <Switch checked={Boolean(form[f.name])} onCheckedChange={(v) => setForm((s) => ({ ...s, [f.name]: v }))} />
                    <span className="text-sm text-muted-foreground">{Boolean(form[f.name]) ? "Ativo" : "Inativo"}</span>
                  </div>
                ) : f.type === "select" ? (
                  <Select value={(form[f.name] as string) ?? ""} onValueChange={(v) => setForm((s) => ({ ...s, [f.name]: v }))}>
