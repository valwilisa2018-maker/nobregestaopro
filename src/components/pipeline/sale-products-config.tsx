...
const typeMeta: Record<Exclude<ProductType, "">, { label: string; icon: typeof Box; color: string }> = {
  physical: { label: "Físico", icon: Box, color: "bg-blue-500/15 text-blue-500 border-blue-500/30" },
  digital: { label: "Digital", icon: Cloud, color: "bg-purple-500/15 text-purple-500 border-purple-500/30" },
  service: { label: "Serviço", icon: Wrench, color: "bg-emerald-500/15 text-emerald-500 border-emerald-500/30" },
};
...
export function SaleProductsConfig() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from("sale_products" as never)
      .select("*").eq("user_id", u).order("created_at", { ascending: false });
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        const { data, error } = await supabase.from("sale_products" as never).insert(payload as never).select("id").single();
        if (error) throw error;
  ...
  const remove = async (id: string) => {
    ...
    try { localStorage.removeItem(IMG_KEY(uid, id)); } catch { /* noop */ }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Cadastre produtos físicos, digitais ou serviços com todos os detalhes.</p>
        </div>
            ...
            return (
              <Card key={p.id} className="relative overflow-hidden group">
                ...
                <CardContent className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        ...
                        {p.sku && <Badge variant="outline" className="text-[10px] font-mono">SKU {p.sku}</Badge>}
                        {p.stock_quantity != null && (
                          <Badge variant="outline" className={`text-[10px] ${p.stock_quantity <= 0 ? "text-destructive border-destructive/40" : ""}`}>
                  ...
                  <p className="text-xl font-black tabular-nums text-primary">{formatBRL(p.price_cents)}<span className="text-xs font-normal text-muted-foreground">/{p.unit || "un"}</span></p>
        ...
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto border-slate-800/70 bg-gradient-to-b from-slate-950 to-slate-900 text-slate-100 shadow-2xl">
          ...
          <Tabs defaultValue="basic" className="space-y-5">
            <TabsList className="grid grid-cols-4 w-full h-auto gap-1 rounded-xl bg-slate-900/60 p-1 ring-1 ring-slate-800/80">
              ...
              <TabsTrigger value="details" className="gap-2 rounded-lg py-2.5 data-[state=active]:bg-gradient-to-br data-[state=active]:from-blue-600/30 data-[state=active]:to-indigo-600/20 data-[state=active]:text-blue-300 data-[state=active]:ring-1 data-[state=active]:ring-blue-500/50">
                <ListOrdered className="h-4 w-4" /> Detalhes
              </TabsTrigger>
              <TabsTrigger value="specific" className="gap-2 rounded-lg py-2.5 data-[state=active]:bg-gradient-to-br data-[state=active]:from-blue-600/30 data-[state=active]:to-indigo-600/20 data-[state=active]:text-blue-300 data-[state=active]:ring-1 data-[state=active]:ring-blue-500/50">
                <Pencil className="h-4 w-4" /> Específico
              </TabsTrigger>
              <TabsTrigger value="image" className="gap-2 rounded-lg py-2.5 data-[state=active]:bg-gradient-to-br data-[state=active]:from-blue-600/30 data-[state=active]:to-indigo-600/20 data-[state=active]:text-blue-300 data-[state=active]:ring-1 data-[state=active]:ring-blue-500/50">
            ...
            <TabsContent value="details" className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid gap-2">
                  <Label>Marca</Label>
                  <Input placeholder="Ex: Apple" value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} />
                </div>
                <div className="grid gap-2">
                  <Label>SKU / Código</Label>
                  <Input placeholder="Ex: PROD-001" value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} />
                </div>
                <div className="grid gap-2">
            ...
            <TabsContent value="specific" className="space-y-3">
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Input type="number" min={0} step="0.1" value={form.width_cm} onChange={(e) => setForm({ ...form, width_cm: e.target.value })} />
                    </div>
                    <div className="grid gap-2">
                      <Label>Altura (cm)</Label>
                      <Input type="number" min={0} step="0.1" value={form.height_cm} onChange={(e) => setForm({ ...form, height_cm: e.target.value })} />
                    </div>
                    <div className="grid gap-2">
                      <Label>Comprimento (cm)</Label>
                      <Input type="number" min={0} step="0.1" value={form.length_cm} onChange={(e) => setForm({ ...form, length_cm: e.target.value })} />
                    </div>
                  </div>
                </>
              )}
                ...
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Input type="number" min={0} value={form.service_duration_minutes} onChange={(e) => setForm({ ...form, service_duration_minutes: e.target.value })} />
                    </div>
                    <div className="grid gap-2">
              ...
              {!form.product_type && (
                <p className="text-xs text-muted-foreground text-center py-8">Selecione um tipo (Físico, Digital ou Serviço) na aba <b>Básico</b> para ver os campos específicos.</p>
              )}
            </TabsContent>
            ...
            <TabsContent value="image" className="space-y-3">
              ...
              {image ? (
                <div className="relative">
                  <img src={image} alt="Preview" className="w-full max-h-64 object-contain rounded-lg border bg-muted" />
                  <Button size="sm" variant="destructive" className="absolute top-2 right-2" onClick={() => setImage(null)}>
