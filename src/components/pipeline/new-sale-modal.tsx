...
export function NewSaleModal({
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto border-slate-800/70 bg-gradient-to-b from-background to-card text-foreground shadow-2xl">
        <DialogHeader>
        ...
        <div className="space-y-5">
          <div className="grid gap-2">
            <Label className="text-foreground font-semibold flex items-center gap-2"><User className="h-3.5 w-3.5 text-blue-400" />Vincular a contato existente (opcional)</Label>
            <Select value={contactId || "none"} onValueChange={(v) => v === "none" ? setContactId("") : pickContact(v)}>
              <SelectTrigger className="h-11 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60"><SelectValue placeholder="Selecionar contato" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><User className="h-3.5 w-3.5 text-blue-400" />Nome do cliente <span className="text-blue-400">*</span></Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome completo"
                className="h-11 bg-card/60 border-slate-800 text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:border-blue-500/60" />
            </div>
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-blue-400" />Telefone / WhatsApp</Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><Building2 className="h-3.5 w-3.5 text-blue-400" />Empresa</Label>
              <Input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Razão social / Nome fantasia"
                className="h-11 bg-card/60 border-slate-800 text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:border-blue-500/60" />
            </div>
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><IdCard className="h-3.5 w-3.5 text-blue-400" />CPF / CNPJ</Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><CreditCard className="h-3.5 w-3.5 text-blue-400" />Forma de pagamento</Label>
              <Select value={payment} onValueChange={setPayment}>
                <SelectTrigger className="h-11 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><Layers className="h-3.5 w-3.5 text-blue-400" />Etapa inicial <span className="text-blue-400">*</span></Label>
              <Select value={stageId} onValueChange={setStageId}>
                <SelectTrigger className="h-11 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60"><SelectValue placeholder="Escolha a etapa" /></SelectTrigger>
                <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><FileText className="h-3.5 w-3.5 text-blue-400" />Nº da Nota Fiscal</Label>
              <Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} placeholder="Ex: 000123"
                className="h-11 bg-card/60 border-slate-800 text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:border-blue-500/60" />
            </div>
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2"><UserSquare2 className="h-3.5 w-3.5 text-blue-400" />Vendedor</Label>
          ...
          <div className="grid gap-2">
            <Label className="text-foreground font-semibold flex items-center gap-2"><Wallet className="h-3.5 w-3.5 text-blue-400" />Status do pagamento</Label>
              ...
              {([
                ...
                <Button key={o.v} type="button" size="sm"
                  ...
                  className={paymentStatus === o.v
                    ? "bg-gradient-to-br from-blue-600 to-indigo-600 text-foreground ring-1 ring-blue-500/60 shadow-[0_0_20px_-4px_rgba(59,130,246,0.6)] hover:from-blue-500 hover:to-indigo-500"
                    : "bg-card/60 border border-slate-800 text-muted-foreground hover:bg-muted/60 hover:text-foreground"}>
                  {o.label}
                </Button>
              ))}
            </div>
            {paymentStatus === "partial" && (
              <div className="grid gap-1 mt-2 rounded-lg border border-slate-800 bg-card/40 p-3">
                ...
                <Input type="number" min={0} step="0.01" value={downPayment}
                  ...
                  className="h-10 bg-card/60 border-slate-800 text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60" />
          ...
          <div className="space-y-2 rounded-xl border border-slate-800 bg-card/40 p-4">
            <div className="flex items-center justify-between">
              <Label className="text-foreground font-semibold">Produtos <span className="text-blue-400">*</span></Label>
              <Button size="sm" onClick={addItem} type="button"
                className="bg-gradient-to-br from-blue-600 to-indigo-600 text-foreground hover:from-blue-500 hover:to-indigo-500 shadow-[0_0_20px_-4px_rgba(59,130,246,0.6)]">
                <Plus className="h-3 w-3" /> Adicionar
              </Button>
            </div>
            {items.length === 0 && (
              <p className="text-xs text-muted-foreground py-6 text-center border border-dashed border-slate-800 rounded-lg">
            ...
            {items.map((i) => (
              <div key={i.key} className="grid grid-cols-[1fr_80px_110px_36px] gap-2 items-end">
                <Select value={i.product_id ?? ""} onValueChange={(v) => pickProduct(i.key, v)}>
                  <SelectTrigger className="h-10 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60"><SelectValue placeholder="Produto" /></SelectTrigger>
                  <SelectContent>
                ...
                <Input
                  ...
                  className="h-10 bg-card/60 border-slate-800 text-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60"
                />
                <Input
          ...
          <div className="grid gap-2">
            <Label className="text-foreground font-semibold flex items-center gap-2"><StickyNote className="h-3.5 w-3.5 text-blue-400" />Observação</Label>
            <Textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Detalhes da venda…" rows={3}
              className="bg-card/60 border-slate-800 text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-blue-500/60 focus-visible:border-blue-500/60" />
          </div>
        ...
        <DialogFooter className="gap-2 border-t border-slate-800 pt-4">
          ...
          <Button onClick={save} disabled={saving}
            className="bg-gradient-to-br from-blue-600 to-indigo-600 text-foreground hover:from-blue-500 hover:to-indigo-500 shadow-[0_0_24px_-4px_rgba(59,130,246,0.7)]">
