...
export function NewSaleModal({
  ...
  const save = async () => {
    ...
    if (items.some((i) => !i.product_name.trim())) {
      toast.error("Selecione o produto de todos os itens");
      return;
    }
    if (!stageId) {
      toast.error("Escolha a etapa do pipeline");
      return;
    }
        ...
        .insert({
          ...
          down_payment_cents:
            paymentStatus === "partial"
              ? Math.round((parseFloat(downPayment) || 0) * 100)
              : paymentStatus === "paid"
                ? total
                : 0,
          note: note || null,
        ...
        <div className="space-y-5">
          <div className="grid gap-2">
            <Label className="text-foreground font-semibold flex items-center gap-2">
              ...
              Vincular a contato existente (opcional)
            </Label>
            <Select
              value={contactId || "none"}
              onValueChange={(v) => (v === "none" ? setContactId("") : pickContact(v))}
            >
              <SelectTrigger className="h-11 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2">
                <IdCard className="h-3.5 w-3.5 text-blue-400" />
                CPF / CNPJ
              </Label>
              <Input
                value={document}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2">
                ...
                Etapa inicial <span className="text-blue-400">*</span>
              </Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              <Label className="text-foreground font-semibold flex items-center gap-2">
                <UserSquare2 className="h-3.5 w-3.5 text-blue-400" />
                Vendedor
              </Label>
              <Input
                value={sellerName}
              ...
              <div className="grid gap-1 mt-2 rounded-lg border border-slate-800 bg-card/40 p-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  ...
                  <strong className="tabular-nums text-blue-300">
                    {formatBRL(
                      Math.max(0, total - Math.round((parseFloat(downPayment) || 0) * 100)),
                    )}
                  </strong>
                </p>
              </div>
          ...
          <div className="space-y-2 rounded-xl border border-slate-800 bg-card/40 p-4">
            <div className="flex items-center justify-between">
              <Label className="text-foreground font-semibold">
                Produtos <span className="text-blue-400">*</span>
              </Label>
              <Button
                size="sm"
                onClick={addItem}
            ...
            {items.map((i) => (
              <div key={i.key} className="grid grid-cols-[1fr_80px_110px_36px] gap-2 items-end">
                <Select value={i.product_id ?? ""} onValueChange={(v) => pickProduct(i.key, v)}>
                  <SelectTrigger className="h-10 bg-card/60 border-slate-800 text-foreground focus:ring-2 focus:ring-blue-500/60">
                    <SelectValue placeholder="Produto" />
                  </SelectTrigger>
                  <SelectContent>
                    {products.length === 0 && (
                      <div className="p-2 text-xs text-muted-foreground">
                        Nenhum produto configurado. Vá à aba "Produtos".
                      </div>
                    )}
                    {products.map((p) => (
                ...
                <Input
                  ...
                  value={i.quantity}
                  onChange={(e) =>
                    patchItem(i.key, { quantity: Math.max(1, parseInt(e.target.value) || 1) })
                  }
