  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.blocked) {
      const { error } = await context.supabase.rpc("master_suspend_account", {
        ...
        _reason: data.reason ?? "bloqueado pelo Master",
      });
      if (error) throw new Error(error.message);
    } else {
      const { error } = await context.supabase.rpc("master_reactivate_account", {
        _user_id: data.userId,
      });
      if (error) throw new Error(error.message);
    }
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.tokens && data.tokens > 0) {
      const { error } = await context.supabase.rpc("master_grant_credits", {
        _user_id: target.userId,
        _tokens: data.tokens,
        _reason: data.reason ?? "ativação manual por e-mail",
      });
      if (error) throw new Error(error.message);
