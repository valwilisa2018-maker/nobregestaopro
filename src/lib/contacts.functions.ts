...
export const upsertContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = { user_id: context.userId, name: data.name ?? null, phone, status: data.status, notes: data.notes ?? null };
    if (data.id) {
      const { error } = await context.supabase.from("contacts").update(payload).eq("id", data.id).eq("user_id", context.userId);
      if (error) throw new Error(error.message);
      return { id: data.id };
    }
