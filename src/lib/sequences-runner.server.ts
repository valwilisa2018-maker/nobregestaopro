...
type Enrollment = {
  id: string; sequence_id: string; user_id: string; contact_id: string | null;
  phone: string; status: string; current_step: number;
  next_run_at: string | null; retry_count: number; last_sent_at: string | null;
};
...
function inWindow(now: Date, ws: string | null, we: string | null, weekdays: number[] | null | undefined): boolean {
  const wd = weekdays && weekdays.length ? weekdays : [0, 1, 2, 3, 4, 5, 6];
  if (!wd.includes(now.getDay())) return false;
  const s = parseHM(ws), e = parseHM(we);
  if (!s || !e) return true;
  const mins = now.getHours() * 60 + now.getMinutes();
  return mins >= s[0] * 60 + s[1] && mins <= e[0] * 60 + e[1];
}

function nextAllowedSlot(from: Date, ws: string | null, we: string | null, weekdays: number[] | null | undefined): Date {
  const wd = weekdays && weekdays.length ? weekdays : [0, 1, 2, 3, 4, 5, 6];
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const dayStart = new Date(day); dayStart.setHours(start[0], start[1], 0, 0);
    const dayEnd = new Date(day); dayEnd.setHours(end[0], end[1], 0, 0);
    if (i === 0 && cur > dayEnd) continue;
    if (i === 0 && cur >= dayStart) return cur;
    return dayStart;
  }
  const fallback = new Date(from); fallback.setDate(fallback.getDate() + 1); return fallback;
}

async function logEvent(db: Db, en: Enrollment, step: Step | null, type: string, message?: string, data?: unknown) {
  try {
    await db.from("sequence_events").insert({
      enrollment_id: en.id, sequence_id: en.sequence_id, user_id: en.user_id,
      step_id: step?.id ?? null, step_position: step?.position ?? null,
      type, message: message ?? null, data: (data ?? {}) as never,
    } as never);
  } catch { /* best-effort */ }
}
...
export async function processDueEnrollments(db: Db, limit = 25) {
  const now = new Date();
  const { data: due } = await db.from("sequence_enrollments")
    .select("*")
  ...
  let processed = 0, sent = 0, errors = 0, waiting = 0, completed = 0;

  for (const en of list) {
    processed++;
    const { data: seqRow } = await db.from("sequences")
      .select("*").eq("id", en.sequence_id).maybeSingle();
    ...
    const { data: stepRow } = await db.from("sequence_steps")
      .select("*").eq("sequence_id", en.sequence_id).eq("position", en.current_step).maybeSingle();
    ...
    if (!inWindow(now, winStart, winEnd, winDays)) {
      ...
      await db.from("sequence_enrollments").update({
        status: "out_of_window", next_run_at: nextSlot.toISOString(),
      } as never).eq("id", en.id);
      await logEvent(db, en, step, "out_of_window",
    ...
    if (!connId) {
      const { data: firstConn } = await db.from("connections")
        .select("id").eq("user_id", en.user_id).order("created_at", { ascending: true })
        .limit(1).maybeSingle();
      connId = (firstConn as { id?: string } | null)?.id ?? null;
    }
    if (!connId || !step.flow_id) {
      await logEvent(db, en, step, "error", !connId ? "Sem instância WhatsApp" : "Etapa sem fluxo vinculado");
      await db.from("sequence_enrollments").update({
    ...
    const { data: conn } = await db.from("connections")
      .select("id,user_id,url_api,api_key,instance_name").eq("id", connId).maybeSingle();
    const { data: flow } = await db.from("flows")
      .select("id,definition").eq("id", step.flow_id).maybeSingle();
    if (!conn || !flow?.definition) {
      await logEvent(db, en, step, "error", "Fluxo ou conexão inválidos");
      await db.from("sequence_enrollments").update({
        status: "error", last_error: "invalid_flow_or_conn",
      } as never).eq("id", en.id);
      errors++; continue;
    }
    ...
    let convoId: string | null = null;
    {
      const { data: existing } = await db.from("conversations")
        .select("id").eq("user_id", en.user_id).eq("connection_id", connId)
        .eq("contact_wa_id", recipient).maybeSingle();
      convoId = (existing as { id?: string } | null)?.id ?? null;
      if (!convoId) {
        const { data: created } = await db.from("conversations").insert({
          user_id: en.user_id, connection_id: connId,
          contact_id: en.contact_id, contact_wa_id: recipient,
          contact_phone: en.phone, last_message_at: now.toISOString(),
    ...
    try {
      await db.from("sequence_enrollments").update({ status: "running" } as never).eq("id", en.id);
      const { runFlow } = await import("@/lib/flow-runner.server");
      const def = (flow as { definition: unknown }).definition as { nodes: unknown[]; edges: unknown[] };
      await runFlow({
        db,
        conn: {
        ...
        state: { flow_id: step.flow_id, current_node: null, variables: { telefone: en.phone }, awaiting: null },
        flowId: step.flow_id,
        conversationId: convoId,
        userId: en.user_id,
      });
      const nextPos = en.current_step + 1;
      const { data: nextStep } = await db.from("sequence_steps")
        .select("id,delay_value,delay_unit,end_sequence").eq("sequence_id", en.sequence_id)
        .eq("position", nextPos).maybeSingle();
      if (!nextStep || step.end_sequence) {
        await db.from("sequence_enrollments").update({
          status: "completed", completed_at: now.toISOString(),
          last_sent_at: now.toISOString(), retry_count: 0, last_error: null,
        ...
        await db.from("sequence_enrollments").update({
          status: "scheduled", current_step: nextPos,
          next_run_at: nextRun.toISOString(),
          last_sent_at: now.toISOString(), retry_count: 0, last_error: null,
        } as never).eq("id", en.id);
        await logEvent(db, en, step, "sent", `Etapa ${step.position + 1} enviada; próxima em ${nextRun.toISOString()}`);
      }
      sent++;
    } catch (err) {
      ...
      if (nextRetry < step.max_retries && step.on_error === "retry") {
        ...
        await db.from("sequence_enrollments").update({
          status: "scheduled", next_run_at: retryAt.toISOString(),
          retry_count: nextRetry, last_error: msg,
        } as never).eq("id", en.id);
        await logEvent(db, en, step, "error_retry", `Erro (tentativa ${nextRetry}): ${msg}`);
      } else if (step.on_error === "skip") {
        await db.from("sequence_enrollments").update({
          status: "scheduled", current_step: en.current_step + 1,
          next_run_at: now.toISOString(), retry_count: 0, last_error: msg,
        } as never).eq("id", en.id);
        await logEvent(db, en, step, "skipped", `Etapa pulada por erro: ${msg}`);
      } else if (step.on_error === "remove") {
        await db.from("sequence_enrollments").update({
          status: "cancelled", completed_at: now.toISOString(), last_error: msg,
        } as never).eq("id", en.id);
        await logEvent(db, en, step, "removed", `Removido por erro: ${msg}`);
      } else {
        await db.from("sequence_enrollments").update({
          status: "paused", last_error: msg, retry_count: nextRetry,
        } as never).eq("id", en.id);
 ...
 */
export async function tryKeywordEnroll(db: Db, args: {
  userId: string; phone: string; text: string;
}) {
  const { userId } = args;
  ...
  const list = (seqs ?? []) as Array<Seq & {
    keywords: string[]; keyword_match: string;
    keyword_ignore_case: boolean; keyword_ignore_accents: boolean;
  }>;
  ...
  for (const s of list) {
    ...
    const { data: existing } = await db.from("sequence_enrollments")
      .select("id,status").eq("sequence_id", s.id).eq("phone", phone).maybeSingle();
    if (existing) {
      const policy = (s as unknown as { reenroll_policy: string }).reenroll_policy;
      if (policy === "skip") continue;
      await db.from("sequence_enrollments").update({
        status: "scheduled", current_step: 0, next_run_at: now,
        entry_source: "keyword", entry_at: now, retry_count: 0, last_error: null,
        completed_at: null,
      } as never).eq("id", (existing as { id: string }).id);
      await db.from("sequence_events").insert({
        enrollment_id: (existing as { id: string }).id, sequence_id: s.id,
        user_id: userId, type: "keyword_enroll", message: "Reinscrição por palavra-chave",
      } as never);
    } else {
      const { data: ins } = await db.from("sequence_enrollments").insert({
        sequence_id: s.id, user_id: userId, contact_id: contactId,
        phone, status: "scheduled", current_step: 0, next_run_at: now,
        entry_source: "keyword",
      } as never).select("id").single();
      if (ins) {
        await db.from("sequence_events").insert({
          enrollment_id: (ins as { id: string }).id, sequence_id: s.id,
          user_id: userId, type: "keyword_enroll", message: "Inscrito por palavra-chave",
        } as never);
      }
    }
    matched++;
