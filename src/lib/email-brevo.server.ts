...
function renderTemplate(opts: {
  ...
  const banner =
    bannerImg && ctaUrl
      ? `<a href="${ctaUrl}" target="_blank" style="display:block;text-decoration:none">${bannerImg}</a>`
      : bannerImg;
  const cta =
    ctaLabel && ctaUrl
...
export async function sendBrevoEmail(input: {
  ...
  if (apiKey.startsWith("xsmtpsib-")) {
    throw new Error(
      "A chave configurada é SMTP da Brevo (começa com xsmtpsib-) e não funciona aqui. Configure uma API Key v3 da Brevo, que começa com xkeysib-.",
    );
  }
  if (!apiKey.startsWith("xkeysib-")) {
    throw new Error("Chave Brevo inválida. Use uma API Key v3 da Brevo, que começa com xkeysib-.");
  }
  if (!input.settings.sender_email)
    throw new Error("Remetente não configurado. Configure em Admin › E-mails.");
...
export async function checkBrevoSender(settings: EmailSettings | null) {
  ...
  if (!apiKey)
    return {
      ...
      message: "Chave Brevo não configurada.",
    };
  if (apiKey.startsWith("xsmtpsib-"))
    return {
      ok: false as const,
      status: "smtp_key" as const,
      message: "A chave salva é SMTP; use uma API Key v3 iniciada por xkeysib-.",
    };
  if (!apiKey.startsWith("xkeysib-"))
    return { ok: false as const, status: "invalid_key" as const, message: "Chave Brevo inválida." };
  if (!settings?.sender_email)
    return {
      ok: false as const,
      status: "missing_sender" as const,
      message: "Remetente não configurado.",
    };
  ...
  if (!res.ok) {
    return {
      ...
      message: `Brevo [${res.status}]: ${text}`,
    };
  }
  const payload = JSON.parse(text || "{}") as {
    senders?: Array<{ email?: string; name?: string; active?: boolean }>;
  };
  const sender = payload.senders?.find(
    (s) => s.email?.toLowerCase() === settings.sender_email?.toLowerCase(),
  );
  if (!sender) {
    return {
      ok: false as const,
      status: "sender_not_found" as const,
      message: `O remetente ${settings.sender_email} não aparece como remetente verificado na Brevo.`,
    };
  }
  if (sender.active === false) {
    return {
      ok: false as const,
      status: "sender_inactive" as const,
      message: `O remetente ${settings.sender_email} existe na Brevo, mas ainda não está ativo/verificado.`,
    };
  }
  return {
    ok: true as const,
    status: "ready" as const,
    message: `API e remetente ${settings.sender_email} estão prontos na Brevo.`,
  };
}
...
export async function sendSignupWelcome(email: string, name: string | undefined, ctaUrl?: string) {
  ...
  await sendBrevoEmail({
    to: email,
    toName: name,
    subject: settings.signup_subject,
    html,
    settings,
  });
