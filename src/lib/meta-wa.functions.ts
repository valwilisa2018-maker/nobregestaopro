  ...
  .handler(async ({ data, context }) => {
    ...
    const r = await graph<{ display_phone_number?: string; verified_name?: string }>(
      ...
      cfg.access_token,
      {
        ...
        qs: { fields: "display_phone_number,verified_name,quality_rating" },
      },
    );
    await supabase
      .from("meta_wa_configs")
      ...
      .eq("user_id", userId);
    return r.ok
      ? { ok: true, phone: r.data?.display_phone_number, name: r.data?.verified_name }
...
export const syncMetaTemplates = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!cfg?.business_account_id || !cfg.access_token)
      return { ok: false, error: "faltando WABA ID ou token" };
    const r = await graph<{
      data: Array<{
        id: string;
        name: string;
...
export const createMetaTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const r = await graph<{ id: string; status: string; category: string }>(
      `/${cfg.business_account_id}/message_templates`,
      cfg.access_token,
      {
        method: "POST",
        version: cfg.graph_version,
        body: { name: data.name, language: data.language, category: data.category, components },
      },
    );
    if (!r.ok || !r.data) return { ok: false, error: r.error };
    await supabase.from("meta_wa_templates").insert({
      user_id: userId,
      config_id: cfg.id,
      meta_template_id: r.data.id,
      name: data.name,
      language: data.language,
      category: r.data.category ?? data.category,
...
export const deleteMetaTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: t } = await supabase
      ...
      .select("*, meta_wa_configs(access_token,business_account_id,graph_version)")
