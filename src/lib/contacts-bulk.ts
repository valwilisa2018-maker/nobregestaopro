...
export async function parseContactsFile(file: File): Promise<ContactRow[]> {
  ...
  return rows
    .map((r) => {
      ...
      const phone = get(["telefone", "phone", "numero", "número", "celular", "whatsapp"]).replace(
        /\D/g,
        "",
      );
...
export function exportContactsXLSX(rows: ContactRow[]) {
  const data = [
    HEADERS,
    ...rows.map((r) => [r.name ?? "", r.phone, r.status ?? "active", r.notes ?? ""]),
  ];
...
export function exportContactsCSV(rows: ContactRow[]) {
  ...
  const csv = [
    HEADERS.join(","),
    ...rows.map((r) =>
      [r.name ?? "", r.phone, r.status ?? "active", r.notes ?? ""]
        .map((c) => esc(String(c)))
        .join(","),
    ),
  ].join("\n");
